...
- Once the **wxWidgets** folder is cloned and opened in *Visual Studio*:
  - Open `r menu(c('Project', 'CMake Settings for wxWidgets'))`
    - Uncheck **wxBUILD_SHARED**
    - Set **wxBUILD_OPTIMISE** to "ON"
    - Set the configuration type to "Release"
    - Save your changes
  - Select `r menu(c('Build', 'Install wxWidgets'))` (builds and then copies the header, lib, and *CMake* files to the prefix folder)
