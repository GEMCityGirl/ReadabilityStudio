...
To create a debug build, pass `-DCMAKE_BUILD_TYPE=Debug` during the configuration stage,
and `--config Debug` during the build stage. Additionally, pass `-DUSE_ADDRESS_SANITIZE=TRUE`
to enable AddressSanitizer for debug builds.
:::
