...
- Add its definition to `BaseProject::InitializeStandardReadabilityTests()` (or proper subfunction)
...
- If a graphical test (e.g., Fry), then also do the following:
  ...
  - Load results in `BatchProjectDoc::LoadScores()` if graph handles its own scoring (e.g., Fry)
...
- Add this `AddXXXTest()` function to the standard test function pointer map table in\
  `BaseProject::OnItemSelected()`
