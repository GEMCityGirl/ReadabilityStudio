...
- Add saving logic to `ToolsOptionsDlg::SaveOptions()` (and related `SaveProjectXXXOptions()` function, if applicable)
...
- If not already present, create a `wxRibbonPanel*` variable from calling\
  ...
  - In the same function, (if not present) add code to show the graph's panel
    when the selected item is of that graph's type. Also, add code to toggle the button's state (if applicable)
...
- Add internal name constant to `r file_path(c("src", "results_format", "readability_messages.h"))`.
...
- If a graphical test (e.g., Fry), then also do the following:
  ...
  - Otherwise, if the score is calucalated by a formula and the graph is a supplement to visualize that score (e.g., Crawford), then add a virtual `AddXXXGraph()` function to `r file_path(c("src", "projects", "base_project.h"))` and call that from the test's `AddXXXTest()` function in `r file_path(c("src", "projects", "base_project.cpp"))`
...
- Add an icon to `r file_path(c("resources", "images", "tests"))` in SVG format using the name of the test's string ID
...
- In `ProjectDoc::DisplayWordsBreakdown()`, add logic to add a page for the data provider and show it in a list control.
  (Should involved showing or hiding these windows if the test is included.)
...
- `r file_path(c("cmake", "includes", "words.cmake"))`: the word lists to be packaged into "res.wad" (the distributable resource file)

The file `r file_path(c("cmake", "list-files.cmake"))` gathers and filters the that should be included.
