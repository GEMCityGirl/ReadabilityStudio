...
In this example, we will write and edit a document from within *Readability Studio* itself. Click the `r keys("New")` button on the **Home** tab to open the **New Project** wizard.
...
knitr::include_graphics("images/termchecklistgrammar.png")
knitr::include_graphics("images/editorwithcontent.png")
```
:::
...
In the next example (see \@ref(external-editing)), we will edit our document from another program and view the changes in *Readability Studio* in real-time.

\newpage

## Editing from Other Programs {#external-editing}

Continuing from the previous example (see \@ref(rs-as-an-editor)), we have copied the text from the editor into another program.
(In this example, *LibreOffice* was used, but another other editor will work as well.)

![](images/Nongenerated/term-checklist-in-external-editor.png)
...
Because this project was created from entered text, it is not currently linked to a document.
Under *Real-time update* is a text box where you can enter the path to a document. Click the button next to this text box and select the file that you just saved from *LibreOffice*.
Also, check the option *Real-time update*, which will be discussed later.
...
```{r echo=F, fig.show = 'hold', out.width='50%'}
knitr::include_graphics("images/termchecklistgrammar.png")
knitr::include_graphics("images/Nongenerated/term-checklist-in-external-editor.png")
```
...
Once you are finished editing, you can close the external editor and continue reviewing the results in *Readability Studio*.

::: {.tipsection data-latex=""}
To reopen the document in your default editor, click `r keys('Edit Document')` from the **Document** tab on the ribbon.
:::
