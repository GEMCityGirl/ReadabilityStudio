...
+-----------------------------------------+------------------------------------------------------+
| Enable verbose logging                  | Hold down `r keys('Shift')` (`r os_logo('windows')`)\ or `r keys('\\shift')` (`r os_logo('apple')`)\ while the program loads |
+-----------------------------------------+------------------------------------------------------+
:::
...
+-----------------------------------------+------------------------------------------------------+
| Show/hide the ribbon                    | `r keys(c('Ctrl','F1'))` (`r os_logo('windows')`)\ or `r keys(c('\\cmd','F1'))` (`r os_logo('apple')`) |
+-----------------------------------------+------------------------------------------------------+
| Reload the active project               | `r keys('F5')`                                       |
+-----------------------------------------+------------------------------------------------------+
| Create a new project from the clipboard | `r keys(c('Ctrl','V'))` (`r os_logo('windows')`)\ or `r keys(c('\\cmd','V'))` (`r os_logo('apple')`) | 
+-----------------------------------------+------------------------------------------------------+
| Save the active project                 | `r keys(c('Ctrl','S'))` (`r os_logo('windows')`)\ or `r keys(c('\\cmd','S'))` (`r os_logo('apple')`) |
+-----------------------------------------+------------------------------------------------------+
| Print the active window                 | `r keys(c('Ctrl','P'))` (`r os_logo('windows')`)\ or `r keys(c('\\cmd','P'))` (`r os_logo('apple')`) |
+-----------------------------------------+------------------------------------------------------+
| Remove the selected test                | `r keys(c('Ctrl', 'Del'))` (`r os_logo('windows')`)\ or `r keys(c('\\cmd','\\backdel'))` (`r os_logo('apple')`) |
+-----------------------------------------+------------------------------------------------------+
:::
...
::: {.minipage data-latex="{\textwidth}"}
Table: Editing Lists Shortcuts

+-----------------------------------------+------------------------------------------------------+
| To do this                              | Press                                                |
+:========================================+:=====================================================+
| Removes selected item(s) from a list    | `r keys('Del')` (`r os_logo('windows')`)\ or `r keys('\\backdel')` (`r os_logo('apple')`) |
+-----------------------------------------+------------------------------------------------------+
| Edits selected item in a list           | `r keys('Ins')`                                      |
+-----------------------------------------+------------------------------------------------------+
| Add a new item to a list                | `r keys(c('Ctrl','Ins'))` (`r os_logo('windows')`)\ or `r keys(c('\\cmd','Ins'))` (`r os_logo('apple')`) |
+-----------------------------------------+------------------------------------------------------+
:::

# Glossary {#glossary}

## Readability Terms {#readability-glossary}
...
```{r boxplot, echo=F, fig.cap="Visual representation of a box plot's layout, adapted from DeCicco (“Exploring ggplot2 boxplots”)", fig.align='center'}
...
QuartileLabels <- c("25th percentile", "50th percentile\n(median)", "75th percentile")
...
```{r, echo=F, fig.cap='Negative and positive kurtosis', fig.align='center'}
knitr::include_graphics("Images/NonGenerated/kurtosis.png")
```
:::
::::
...
```{r echo=F, fig.align='center', fig.cap="Box plot with no jittering", out.width='75%'}
...
To correct this, a technique called jittering\index{box plots!jittering} is used to adjust the x axis values of overlapping points. Traditionally, jittering\index{jittering|see{box plots,jittering}} uses randomness to adjust the points’ position around both sides of the axis, as such:

```{r, echo=F, fig.cap="Box plot with random jittering", fig.align='center', out.width='75%'}
boxplot(len ~ dose, data = ToothGrowth,
              main = "Guinea Pigs' Tooth Growth",
              xlab = "Vitamin C dose mg",
              ylab = "Tooth length")
...
```{r, echo=F, fig.cap="Box plot with beeswarm jittering", fig.align='center', out.width='75%'}
library(beeswarm)
...
```{r, echo=F, fig.cap='Negative and positive skewness', fig.align='center'}
knitr::include_graphics("Images/NonGenerated/skewness.png")
```
:::
::::
