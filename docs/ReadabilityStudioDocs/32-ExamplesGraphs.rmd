...
Finally, let's change how the bars are drawn. Click the `r keys("Bar Style")` button in the **Edit** section and select **Watercolor**.
...
Finally, let's remove some labels to simplify the graph. Click the `r keys("Labels")` button on the ribbon so that is it depressed.
...
Next, click the `r keys("Background")` button again and select `r menu(c('Image', 'Image...'))`. When the image selection dialog appears, choose an image and click `r keys("OK")`.
...
Click the `r keys("Background")` button and select `r menu(c('Image', 'Effects', 'Oil Painting'))`.

```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/ExampleGraphBackgroundImageEffectChanged.png")
```

By default, the image will be shown fully opaque. For lighter images this will look fine, but darker images (like the one shown above) will make it difficult to read the graph.

One option is to make the image lighter.
Click the `r keys("Background")` button and select **Image Opacity...** from the menu.
On the **Set Opacity** dialog, slide the opacity down to 150 and click `r keys("OK")`.

::: {.minipage data-latex="{\textwidth}"}
Now this image will be translucent and the graph labels will be much easier to read:

```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/ExampleGraphBackgroundImageChangedLighter.png")
```
:::

Another option is to change the graph's text to be white and bold so that it contrasts the background image.
On the ribbon, click the `r keys("Font")` button and select **X Axis...**.
From the font dialog, select bold and white for the color and click `r keys("OK")`.
Do the same for the Y axis and top titles. The graph will now appear as such:

```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/ExampleGraphFontColorChanged.png")
```

Next, we will apply a light color to the plot area. Click the `r keys("Background")` button, select **Plot Area Color...** from the menu, and select a new color (e.g., antique white).
Then, select **Opacity...** from this menu and set it to 25 and click `r keys("OK")`.

Now the plot area will have a light color applied to it:
