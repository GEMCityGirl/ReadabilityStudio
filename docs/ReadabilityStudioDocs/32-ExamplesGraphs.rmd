...
By default, the image will be shown fully opaque. For lighter images this will look fine, but darker images (like the one shown above) will make it difficult to read the bars.

An option we have is to make the image lighter.
Click the `r keys("Background")` button and select `r menu(c('Plot Image', 'Opacity'))`.
On the **Set Opacity** dialog, slide the opacity down to 150 and click `r keys("OK")`.

::: {.minipage data-latex="{\textwidth}"}
Now this image will be translucent and the graph labels will be easier to see:
