## Neue Wiener Sachtextformel 3\index{Neue Wiener Sachtextformel 3|ii} {#neue-wiener-sachtextformel3}

Neue Wiener Sachtextformel 3 (3.nWS) was developed by Bamberger & Vanecek for evaluating German nonfiction (specifically, very light materials up to 5^th^ grade) [-@{bamberger-vanecek}84, 187]. This test returns a grade-level score based on word length and sentence length.

```{definition, nws3, name="Neue Wiener Sachtextformel 3 formula"}
$$
G = 0.2963*((C/W)*100) + 0.1905*(W/S) - 1.1144
$$
```

Where:

```{r, echo=F}
nws3Data <- tibble(col1 = c("G", "C", "X", "M", "S", "W"),
                   col2 = markdown_to_kable_cell(c("Grade level",
                                     "Number of 3+ syllable^a^ words",
                                     "Number of 7+ character^b^ words",
                                     "Number of monosyllabic^c^ words",
                                     "Number of sentences",
                                     "Number of words")))

colnames(nws3Data) <- rep('', ncol(nws3Data))
kableExtra::kbl(nws3Data, booktabs=T, centering=T, align='l', escape=F, linesep="") %>%
  kableExtra::kable_classic(full_width=F) %>%
  footnote(alphabet = c(markdown_to_kable_footnote('“*drei- und mehrsilbigen*”'),
                      markdown_to_kable_footnote('“*mehr als sechs buchstaben*”'),
                      markdown_to_kable_footnote('“*einsilbigen*”')),
           escape=F, threeparttable=T)
```

\newpage