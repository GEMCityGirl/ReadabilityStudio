...
- Greatly enhanced editing documents that are embedded within standard projects. (Refer to \@ref(#rs-as-an-editor) for an example.)
  ...
  - The project window can now be selected while the editor is open. This allows for changing options and adding tests while the editor is active.
  - Added editing features, such as undo/redo and clipboard operations.
...
- The **User agent** option previously available on the **Web Harvester** dialog has been moved to the **General Settings** page of the **Options** dialog.
  This will allow for changing the user agent when reading single web pages, as well as when harvesting pages.
...
- Box plots now use Tukey hinges method for calculating quartiles. (This matches what *R* does.)
...
- Added **Display grouped documents on syllable ruler** feature to the Flesch chart.
- Aggressive exclusion will now make copyright notice exclusion more aggressive also.
...
- Overused Words (x Sentence) now only looks at words containing more than three characters.
- Overused Words (x Sentence) now ignores words that are part of a common phrase (e.g., "*again* and *again*").
