...
Logging behavior is controlled by the [command line](#command-line-options), the **Verbose** button on the **Log Report** dialog, or by holding down the `r keys("Shift")` (`r os_logo('windows')`) or `r keys("\\shift")` (`r os_logo('apple')`) button on your keyword while the program starts.
...
-   Append to the previous log report (the default behavior is to clear the log file every time the program starts)
