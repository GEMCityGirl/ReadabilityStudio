...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/NonGenerated/summer-code-camp-flyer.png")
```
...
## Using a Custom Brush {#custom-brush-example}

In this example, we will use a custom brush for our bar charts. Basically, a custom brush enables us to use stacked images to draw our bars. First, create or open any standard project and select **Words Breakdown** on the sidebar. Next, select the **Word Counts** subitem and note how the bars will be filled rectangles:

```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/GraphBarsRectangular.png")
```

On the **Home** tab of the ribbon, click the `r keys('Bar Style')` button and select **Custom image brush** from the menu:

![](images/RibbonEditBarCharts.png)

Select an image file. After the image is loaded, the bars will be drawn with this image, stacking them on top of each other:
...
This feature can also be applied to box plots and histograms. With any of these graph types selected, perform the same operation from the ribbon to draw with a custom brush.
