...
- [Highlighted text](#text-window-export): `r glue(if_else(IncludeLinux, "HTML, RTF, and Pango (Παν語) (Pango is only available on Linux.)", "HTML and RTF"))`
- Statistic summaries: HTML
- [Test results](#scores-export): HTML
- Word, phrase, and sentence [lists](#export-list): HTML, Text, and `r latex_logo(FALSE)`
...
When [exporting](#exporting)\index{exporting!text reports} a text report\index{text reports!exporting}, the following export formats\index{text reports!exporting!formats} are available:
...
```{r, results='asis', eval=IncludeLinux, echo=F, include=UserManualMode}
glue('**Pango**: A format that can be rendered within libraries such as Cairo or FreeType. (This option is only available on Linux.)')
```
:::
...
When [exporting](#exporting)\index{exporting!graphs} a graph\index{graphs!exporting|(}, you will first be prompted for an image format to export as. The following formats\index{graphs!exporting!formats} are available to choose from:
...
When [exporting](#exporting)\index{exporting!lists} a list\index{lists!exporting}, you will be provided with the following formats\index{lists!exporting!formats} to export as:
...
**LaTeX**: A `r tex_logo(TRUE)`file with the list formatted into a `longtable{}` environment that can be
included in a larger `r latex_logo(TRUE)`document.
:::
...
**Test scores**: Select this option to export all the windows from the [**Readability Scores**](#reviewing-test-scores) section.

**Summary statistics**: Select this option to export all the windows from the [**Summary Statistics**](#reviewing-statistics) section.

**Histograms/box plots**: Select this option to export all [histograms](#reviewing-batch-histograms) and [box plots](#reviewing-batch-box-plots). This option only applies to batch projects.

**Words breakdown**: Select this option to export all the windows from the [**Words Breakdown**](#reviewing-word-breakdowns) section.

**Sentences breakdown**: Select this option to export all the windows from the [**Sentences Breakdown**](#reviewing-sentences-breakdown) section.

**Grammar section**: Select this option to export all the windows from the **Grammar** section.

**Dolch sight words section**: Select this option to export all the windows from the [**Dolch Sight Words**](#reviewing-dolch) section. This option only applies if **Dolch Sight Words** is included in the project.

**Warnings section**: Select this option to export all the project's [warnings](#reviewing-batch-warnings). This option only applies to batch projects.
...
knitr::include_graphics("images/FilterTextPreview.png")
```
