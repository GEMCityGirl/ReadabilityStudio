## Flesch-Kincaid\index{Flesch-Kincaid|(ii} {#sec-flesch-kincaid-test}
...
OptionsDesc <- "This behavior can be adjusted from the [**Options**](#sec-readability-test-options) dialog if you prefer a different numeral syllabizing method."

glue('Note that this test sounds out numerals\'\\index{numerals!Flesch-Kincaid} digits when syllabizing.
For example, "1918" will be counted as four syllables ("one"-"nine"-"one"-"eight"). $if_else(UserManualMode, OptionsDesc, "")^', .open='$', .close='^')
```
...
Refer to \@ref(sec-flesch-kincaid-test).
