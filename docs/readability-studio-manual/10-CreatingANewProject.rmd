...
Note that batch projects are only available with **Professional Edition**.
...
To determine a document's readability you will need to create a new project. First, click the `r keys("New")` button on the **Home** tab to open the **New Project** wizard.
(If you are using the *Professional* edition, select **Standard Project** and click `r keys("OK")`).
...
The layout options are used to describe the format of the document and will help the program select the appropriate [sentence deduction](#sec-options-sentence-deduction) method.
...
From here, you can review the [readability scores](#sec-reviewing-test-scores) and [graphs](#sec-reviewing-readability-graphs), useful document [statistics](#sec-reviewing-statistics), and various lists of [difficult words](#reviewing-word-breakdowns). You can also review the document with its difficult words and [sentences](#reviewing-standard-grammar) highlighted.
...
As you add files to the project, you can also include a helpful description next to any file. Double click in the **Description** column next to any filename, and then type a description to be attached to that file.
When the batch project is loaded, this description will appear in any results window that displays filenames. (Note that for some file formats, the program will fill this field with the document's subject or title information if you leave it blank.)
...
For these situations, check the **Randomly sample** option and enter the percentage of documents to analyze.

You can also filter documents based on content size. Enter into the field **Minimum document word count** the minimum number of words a document must have to be included in the batch.
...
Clicking the `r keys("Forward >")` button will move to the **Document Structure** and **Test Selection** pages. Refer to \@ref(sec-creating-standard-project) for an explanation of these pages.
...
From here, you can review the [readability scores](#sec-reviewing-batch-scores), [box plots](#sec-reviewing-batch-box-plots), [histograms](#sec-reviewing-batch-histograms), [readability graphs](#sec-reviewing-readability-graphs), [Dolch statistics](#sec-reviewing-batch-dolch), and [difficult words](#sec-reviewing-batch-words). You may also want to review any [warnings](#sec-reviewing-batch-warnings) that were encountered while analyzing the documents.
