...
To create a [standard project](#creating-standard-project) for any document in the list, select it and click the `r keys("Subproject")` button on the **Home** tab. This is useful for reviewing a particular document in greater detail. Note that when this standard project is created, it will copy all the project settings from the batch project. (These options may differ from your [global configurations](#options-overview).) \

To remove any document in the list from the project, select it and press `r keys('Del')` (`r os_logo('windows')`) or `r keys('\\backdel')` (`r os_logo('apple')`) on your keyboard.
:::
...
In the histogram below, we can see that 15 documents scored within the 10^th^ grade range for the [Flesch-Kincaid](#flesch-kincaid-test) test:
...
Because we are [rounding](#options-histograms) our scores down (the default), scores such as 9.7 will be rounded down and fall into the previous bin (i.e., bar).
...
The appearance of these histograms can be customized from the [**Histogram**](#options-histograms) page of either the [**Options**](#options-overview) or [**Project Properties**](#options-overview) dialog.

::: {.notesection data-latex=""}
The [binning](#options-histograms) method for a project is only applied to index and predicted-cloze score histograms.
...
Along with the grade-level box plots, a separate box plot for each index test will also be included. Below is a box plot of [Flesch](#flesch-test) scores from a batch:
...
**Key Word Cloud**: Refer to \@ref(reviewing-word-breakdowns).
...
## Summary Statistics {#reviewing-batch-summary-stats}
...
### Reviewing Wording Errors & Misspellings {-#reviewing-batch-wording-errors}
...
### Reviewing Possible Misspellings {-#reviewing-batch-misspellings}
