...
knitr::include_graphics("images/options-dlg.png")
```
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/projectproperties.png")
```
...
**Export**: Click this button to export the current program settings to a file. This includes all your settings on the [**Options**](#sec-options-overview) dialog, your defaults for the [**New Project**](#sec-creating-standard-project) wizard, and your printer headers & footers.
...
- **Warn about documents that contain long incomplete sentences that will be included in the analysis.** This warning is shown when a document being analyzed contains sentences that, although missing terminating punctuation, will be included in the analysis because of their length. To change this behavior, adjust the option [**Include incomplete sentences containing more than [15] words**](#sec-options-text-exclusion).
...
- **Prompt if a custom NDC test's proper noun settings differ from the standard NDC test.** By default, [New Dale-Chall](#sec-dale-chall-test) requires that the first instance of each proper noun be seen as unfamiliar. If you are creating a custom New Dale-Chall test which uses different proper-noun logic, then you will be prompted whether this is your intent.
...
- **Warn when a custom test's numeral settings will be adjusted.** Some tests (e.g., [Harris-Jacobson](#sec-harris-jacobson)) require numerals to be excluded from the analysis. This warning will be shown if a custom test needs its numeral settings adjusted to take such rules into account.
...
**Status**: Enter in this field the status of the document(s) that this project is reviewing (e.g., *DRAFT*). This information will appear in reports when you [export all](#sec-export-all-options).
...
**Real-time update**: Select this option to automatically reload the project as the source document is edited externally.
This allows for editing the document in another program (e.g., *Word*) and having the results update in *Readability Studio* in real time.
(This is only relevant for standard projects and for when the project is linked to a local document.)

::: {.notesection data-latex=""}
The changes will take effect in *Readability Studio* only after you save the document from the other program.
...
### Append Additional Document (e.g., policies, license agreements, addendums) {-#sec-additional-document}
...
### Sentence/Paragraph Deduction\index{sentences!customizing!deduction} {-#sec-options-sentence-deduction}
...
Both of these exceptions can be overridden by the options [**Ignore blank lines**](#sec-options-sentence-deduction) and **Ignore indenting**.
...
Refer to \@ref(sec-line-chaining) for more information about these options.
::::

**Ignore blank lines**: Select this option to not consider [incomplete sentences](#sec-how-text-is-excluded) followed by blank lines as the end of the current sentence and paragraph. If an incomplete sentence is encountered, any blank lines after it will be skipped and the sentence will be merged with the following sentence.

This option is not recommended for documents that contain headers or lists that rely on blank lines to separate them from the narrative text. Refer to \@ref(sec-line-chaining) for more information.
...
**Do not exclude any text**: Select this option to treat all incomplete sentences as regular sentences when calculating statistics. This is only recommended for forms that may contain numerous titles and lists items and little narrative text. Refer to \@ref(sec-fragmented-text) for more information.
...
**Exclude text between**: Selects whether to exclude all text in between a certain pair of tags. For example, select **[ and ]** and any text inside a pair of left and right square brackets will be ignored in the analysis. Along with excluding the bracketed words, any full paragraphs and sentences wrapped inside of these tags will also be excluded (which will be reflected in the statistics). Refer to \@ref(sec-excluding-placeholders) for an example.
...
#### Flesch-Kincaid {-#sec-options-flesch-kincaid}
...
#### Dale-Chall {-#sec-options-dale-chall}
...
#### Harris-Jacobson {-#sec-options-harris-jacobson}
...
Refer to \@ref(sec-grade-scales) for a list of available scales.
...
#### Reading Age Display {-#sec-reading-age}
...
### Summary Report {-#sec-options-summary-report}
...
### Dolch Report {-#sec-options-dolch-report}
...
**Wording Errors & Misspellings**: Check this to include [wording errors and known misspellings](#sec-reviewing-wording-errors) in the results.

**Possible Misspellings**: Check this to include [possible misspellings](#sec-reviewing-misspellings) in the results.

**Repeated Words**: Check this to include [repeated words](#sec-reviewing-repeated-words) in the results.

**Article Mismatches**: Check this to include [article mismatches](#sec-reviewing-article-mismatches) in the results.

**Redundant Phrases**: Check this to include [redundant phrases](#sec-reviewing-redundant-phrases) in the results.

**Overused Words (by Sentence)**: Check this to include [overused words (by sentence)](#sec-reviewing-overused-by-sentence) in the results.

**Wordy Items**: Check this to include [wordy phrases](#sec-reviewing-wordy-items) in the results.

**Clichés**: Check this to include [clichés](#sec-reviewing-cliches) in the results.

**Passive Voice**: Check this to include [passive voice](#sec-reviewing-passive-voice) in the results.

**Conjunction-starting Sentences**: Check this to include [conjunction-starting sentences](#sec-reviewing-conjunction-sentences) in the results.

**Lowercased Sentences**: Check this to include [lowercased sentences](#sec-reviewing-lowercased-sentences) in the results.

### Spell Checker {-#sec-options-spell-check}
...
**Excluded text color**: Click this button to select the color to highlight excluded text in the document viewing area. This is only applicable when [incomplete sentences](#sec-how-text-is-excluded) are being ignored.
...
**Grammar errors color**: Click this button to select the color to highlight grammar errors (e.g., repeated words and misspellings). This highlighting will appear in the [**Grammar**](#sec-reviewing-standard-grammar) document view.
...
### Dolch Sight Words {-#sec-dolch-options}
...
#### Watermarks & Logos {-#sec-watermark-and-logo}
...
Note that watermarks and logos will also appear when you [print](#sec-printing) or [save](#sec-exporting) a graph.

#### Effects {-}

**Stipple image**: Enter into this field the file path to the image used for a stipple brush. A stipple brush can be used to draw stacked images across [bars](#sec-options-bar-charts) and [boxes](#sec-options-box-plots).

**Stipple shape**: Select from here which shape to use for a stipple brush. A stipple brush can be used to draw stacked shape across [bars](#sec-options-bar-charts) and [boxes](#sec-options-box-plots).
...
Also, the labels for these bars and slices will be removed. Finally, the cumulative bar for the simple-words bar group will be removed on the histogram.

::: {.notesection data-latex=""}
If using a stipple shape on the syllable histogram, then the opacity feature will have no effect.
:::
...
### Readability Graphs {-#sec-options-readability-graphs}
...
#### Flesch Chart {-#sec-options-flesch-chart}
...
#### Lix Gauge {-#sec-options-lix-gauge}
...
### Bar Charts {-#sec-options-bar-charts}
...
**Stipple image**: Select this option to draw a stack of images across your bars. The image for this brush is specified on the [**General**](#sec-options-graphs) page.

**Stipple shape**: Select this option to draw a shape across your bars. The shape for this brush is specified on the [**General**](#sec-options-graphs) page.
...
**Common image**: Select this option to draw an image under the graph, with it only showing through were the bars are at.
::::
...
### Histograms {-#sec-options-histograms}
...
This option controls how [floating-point](#sec-grade-level-results) values are rounded when being sorted into bins. You may choose from the following options:
...
### Box Plots\index{box plots!customizing} {-#sec-options-box-plots}
...
**Stipple image**: Select this option to draw a stack of images across your boxes. The image for this brush is specified on the [**General**](#sec-options-graphs) page.

**Stipple shape**: Select this option to draw a shape across your boxes. The shape for this brush is specified on the [**General**](#sec-options-graphs) page.
...
**Common image**: Select this option to draw an image under the graph, with it only showing through were the boxes are at.
::::
