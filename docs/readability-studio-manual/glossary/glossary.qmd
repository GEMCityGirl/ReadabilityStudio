...
This is calculated by sorting the values and taking the value in middle if the number of values is odd.
If the number of values is even, then the means of the two middle points is taken.
:::
::::
...
:::
::: {.glsdef data-latex=''}
The variability (or spread) of the values. A low standard deviation indicates that most values are close to the data's mean.
