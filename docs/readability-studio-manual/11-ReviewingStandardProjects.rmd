...
To remove any test from this list, select it and press `r keys(c('Ctrl', 'Del'))` (`r os_logo('windows')`) or `r keys(c('\\cmd', '\\backdel'))` (`r os_logo('apple')`) on your keyboard. To edit a [custom test](#sec-creating-custom-test), double click on it in this list. To view the help for a standard test, double click on it in this list.
...
You can change both the font and highlighting color by opening the [**Project Properties**](#sec-options-overview) dialog and selecting [**Highlighted Reports**](#options-highlighted-reports).
