...
\definecolor{titlepagecolor}{RGB}{243, 102, 25}

\newgeometry{top=0mm, bottom=0mm, left=0mm, right=0mm}

\tikz[overlay, remember picture, shorten >= -10pt]
{
\coordinate (aux1) at ([yshift=-15pt]current page.north east);
\coordinate (aux2) at ([yshift=-410pt]current page.north east);
\coordinate (aux3) at ([xshift=-4.5cm]current page.north east);
...
$if(pdf-logo)$
\begin{tikzpicture}[overlay, remember picture]
    \node (centerimage) at (current page.south west)[anchor=south west] {\includegraphics[width=1.5cm]{$pdf-logo$}};
\end{tikzpicture}
$endif$
