...
To avoid these issues, it is recommended to include incomplete sentences in the analysis and to only use the [FORCAST](#sec-forcast-test)\index{FORCAST} test. When creating a [new project](#sec-creating-standard-project), there are two ways to implicitly set these options. The first way is to select **non-narrative, fragmented text** when specifying the document structure. The other way is to select **non-narrative form** when specifying the document type.
