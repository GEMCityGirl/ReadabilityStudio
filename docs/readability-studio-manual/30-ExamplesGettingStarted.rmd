...
To do this, click the `r keys("Save")` button on the **Home** tab and use the **Save As...** dialog to save this list to either an HTML or text file. You can also do this with any of the other difficult word lists, such as the Spache unfamiliar words list. Refer to \@ref(sec-vocab-builder-example) for an example.
...
First, you will be prompted to save as either HTML or text (refer to \@ref(sec-export-list) for a discussion of these formats). After specifying the format, the **Save As** dialog will appear. Select where you want to save the file and click `r keys("Save")`.
...
Next, the [**List Export Options**](#sec-export-list) dialog will appear. (By default, all the words will be exported.) To export just the words that you have highlighted, select the option **Export selected rows**. Also, because we are only interested in saving the **Word** column—not the **Syllable Count** or **Frequency Count** columns—enter 1 in the **Columns: from** and **to:** fields. Finally, uncheck the option **Include column headers** because we are only exporting one column.
...
Other useful tools for shortening sentences in *Readability Studio* are the other pages in the **Grammar** section.
Click on the **Grammar** icon on the project sidebar. Click on the [**Repeated Words**](#sec-reviewing-repeated-words) page and note how the phrase *and and* is in the file (obviously a typo).
Next, click on the [**Redundant Phrases**](#sec-reviewing-redundant-phrases) page.
Note how the phrase *mixed together* is redundant and the program is suggesting that it be replaced with *mixed*.
A [**Wordy Items**](#sec-reviewing-wordy-items) and [**Clichés**](#sec-reviewing-cliches) page may also appear, but there do not seem to be any of these in this file.
...
This is discussed in a later example (see \@ref(sec-external-editing)); for now, leave this unchecked.
...
Note how some text is highlighted gray. These are [incomplete sentences](#sec-how-text-is-excluded), and because *Readability Studio*, by default, ignores incomplete sentences, nothing highlighted gray is factored into the analysis.
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/fixed-with-r-sentencemap.png")
```
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/disclaimer-sentencemap.png")
```
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/christmas-carol-sentencemap.png")
```
...
When the project appears, note that only the [FORCAST](#forcast-test) test was performed because it does not use sentence lengths.
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/summer-code-camp-upper-half1.png")
```

For example, we could remove *Proudly presented* or replace it with *Offered*. Scrolling down further:

```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/summer-code-camp-upper-half2.png")
```
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/summer-code-camp-lower-half.png")
```
...
Refer to \@ref(sec-high-level-review-of-disclaimer) and \@ref(sec-addendum-example) for a continuation of this example where the flyer is revised and has an addendum added to it.
...
Continuing from the "Reviewing a Flyer" example (refer to \@ref(sec-reviewing-a-flyer)), our fictitious software company has been purchased by a private equity firm.
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/example-difficult-words-wordcloud.png")
```
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/example-difficult-syllables-histogram.png")
```
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/example-difficult-syllables-histogram-showcased.png")
```
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/example-difficult-syllables-pie-chart.png")
```
...
```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/example-difficult-sentences-heatmap.png")
```
...
From the **Readability** tab, click `r menu(c('Secondary','Fry'))` to add a [Fry](#fry-test) test.
Click on the **Readability Scores** icon on the project sidebar and select **Fry Graph**.

```{r echo=F, fig.align='center', out.width='75%'}
knitr::include_graphics("images/example-difficult-fry-showcased.png")
```
...
Refer to \@ref(sec-addendum-example) for a continuation of this example.
There, this document will be explored in further detail and used as an addendum to the original flyer from \@ref(sec-reviewing-a-flyer).
