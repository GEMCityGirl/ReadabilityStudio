...
Along with offering standard tests, such as [New Dale-Chall](#sec-dale-chall-test)\index{New Dale-Chall!custom test} and [Spache Revised](#sec-spache-test)\index{Spache Revised!custom test}, *Readability Studio* also includes the ability to create your own tests. Many customizable features are offered for new tests, such as being able to select your own familiar-word list and which formula to use.
...
knitr::include_graphics("images/function-browser.png")
```
...
Refer to \@ref(sec-custom-test-functions) for a list of available functions.
...
^a^ Uses the [Dale-Chall text-exclusion](#sec-options-dale-chall) method.\
^b^ Uses the [Harris-Jacobson text-exclusion method](#sec-options-harris-jacobson).\
^c^ Uses the [Gunning Fog](#sec-options-gunning-fog) sentence-counting method.\
...
| MiniWordCount() | Returns the number of miniwords (refer to \@ref(sec-eflaw-test)). |
...
`r keys('ƒx')`  Click this button to display the **Function Browser** dialog. In this dialog, you can browse the available functions and insert them into your formula. Refer to \@ref(sec-custom-test-functions) for a list of available functions.
