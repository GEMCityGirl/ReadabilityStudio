...
- Added real-time updating to standard projects. This enables the project to update itself while you edit the original document in another program.
  (Refer to `r glue("{if_else(UserManualMode, '\\\\@ref(sec-external-editing)', shQuote('Editing from Other Programs'))}")` for an example.)
- Greatly enhanced editing documents that are embedded within standard projects.
  (Refer to `r glue("{if_else(UserManualMode, '\\\\@ref(sec-rs-as-an-editor)', shQuote('Editing within Readability Studio'))}")` for an example.)
...
- **Log Report** window improvements:
  ...
  - Added option to load the log report from the previous run of the program (if ran during the same day).
...
- **Web Harvester** improvements:
  ...
  - Performance improvements; less requests are now sent to servers while crawling them.
  - **Restricted to same folder** domain restriction will now include links from sub-directories of the specified folder.
- Improved dark mode appearance (Windows)
...
- Changing graph backgrounds to black will change font colors from black to white implicitly to make them visible.
...
- Added custom test features:
  - Added feature to create custom tests based on any existing regression-based test.
    (Refer to `r glue("{if_else(UserManualMode, '\\\\@ref(sec-creating-custom-flesch-test)', shQuote('Creating a Modified Flesch Test'))}")` for an example.)
  - Added `GunningFog` option to `SentenceCount()` function. This will return the sentence count using the system's [Gunning Fog method](#sec-options-gunning-fog).
...
- [Overused words (x Sentence)](#sec-reviewing-overused-by-sentence) now excludes acronyms.
...
- Graph [watermarks](#sec-watermark-and-logo) now support the tags `&[DATETIME]`, `&[DATE]`, and `&[TIME]`. These are expanded to their literal values at the time of rendering the graph.
...
- Added option to append an [additional document](#sec-additional-document) to any document that you analyze.
- Rearranged grammar results in order of importance.
- Added new [Sentences Breakdown](#sec-reviewing-sentences-breakdown) section to standard and batch projects. The sentence-length box plot formerly shown in the **Summary Statistics** section is now available here in standard projects. Overly-long sentence lists formerly shown in the **Grammar** section for both types of projects are now in here.
...
- Added new [Overused Words (x Sentence)](#sec-reviewing-overused-by-sentence) list to standard and batch projects.
...
- [**Log Report**](#sec-log-report) button is now always available. (This option was not available unless verbose logging was enabled in previous releases.)
...
- **Independent clause count** statistic now included in the statistics report. An independent clause is a section of text which ends with a period, exclamation mark, question mark, [interrobang](#sec-glossary)\index{interrobangs}, colon, semicolon, or dash.
