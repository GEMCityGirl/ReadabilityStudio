...
### Readability Studio's Unique Features {-#sec-program-unique-features}
...
*Readability Studio* presents an intuitive interface for selecting readability tests. Experts can manually select the tests that they want to perform, while those less familiar with these tests can be guided through a helpful [wizard](#sec-creating-standard-project). This wizard can recommend which tests to include based on the document's type (e.g., text book) or industry (e.g., health care).
...
Multiple-document graphs are also provided in the *Professional* edition of *Readability Studio*. Here, [histograms](#sec-reviewing-batch-histograms) and [box plots](#sec-reviewing-batch-box-plots) show the distribution of a document collection's test scores. This is useful for summarizing the general reading level of a batch of documents. It is also useful for finding problematic documents in a batch (i.e., outliers).

#### Detailed statistics {-}

Numerous sentence and word statistics are provided. This includes a full difficult-words breakdown and word & sentence-length averages. The type of statistics included is also [customizable](#sec-options-statistics).
...
- [wording errors and known misspellings (English only)](#sec-reviewing-wording-errors)
- [possible misspellings (English only)](#sec-reviewing-misspellings)
- [repeated words](#sec-reviewing-repeated-words)
- [article mismatching (English only)](#sec-reviewing-article-mismatches)
- [redundant phrases (English only)](#sec-reviewing-redundant-phrases)
- [wordy items (English only)](#sec-reviewing-wordy-items)
- [clichés (English only)](#sec-reviewing-cliches)
- [overly-long sentences](#sec-reviewing-sentences-breakdown)
- [lowercased sentences](#sec-reviewing-lowercased-sentences)
- [sentences that begin with conjunctions](#sec-reviewing-conjunction-sentences)
- [passive voice (English only)](#sec-reviewing-passive-voice)
...
This enables writers to see how they can improve their Dolch practice materials. This also enables educators to determine which materials are appropriate for Dolch practice.
...
*Readability Studio* provides printing and [exporting](#sec-exporting) capabilities for all test scores, word lists, statistics, graphs, etc. This enables you to easily share and publish your results.

- Test results, statistics, and difficult word lists can all be exported as HTML, text, or `r latex_logo(FALSE)`.
...
Finally, *Readability Studio* is highly configurable. Many options, such as how documents are analyzed and how the results are displayed, are customizable.
For example, you can specify how numeric words (e.g., *10/31/1974*) are [syllabized](#sec-document-analysis).
Other programs either sound out each digit or treat the entire word as one syllable and you cannot change this. With *Readability Studio*, you can toggle this behavior at anytime.
