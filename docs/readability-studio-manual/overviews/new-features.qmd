...
## Readability Studio 2025 {-}
***

### General Enhancements {-}

- Added Lua scripting. This allows for programming your own scripts to automate the application.
...
- `@USER@` placeholder can now be entered in printer headers and footers.
...
- [Combined accents](#diacritics) (i.e., diacritics) are now supported.\
  For example, "e◌́" will be imported as a 'é'.
- [Typographical ligatures](#typographical-ligatures) are now converted to the regular characters.\
  For example, "ﬀ" will be imported as "ff."
- Custom test formulas:
    - New function `IFS`.
    - The functions `MIN`, `MAX`, `SUM`, `AVERAGE`, `OR`, and `AND` now support 24 arguments. (These were limited to 7 arguments previously.)
...
## Readability Studio 2024.0.1 {-}
***

### General Enhancements {-}

- macOS version now runs natively under Apple Silicon (i.e., ARM64 processors).
...
- Fixed empty dialog intermittently appearing under macOS.
...
- Greatly enhanced editing documents that are embedded within standard projects.
  (Refer to [@sec-rs-as-an-editor]{.content-visible when-profile="online"}["Editing within Readability Studio"]{.content-hidden when-profile="online"} for an example.)
...
- Added custom test features:
  ...
  - Added `GunningFog` option to `SentenceCount()` function. This will return the sentence count using the system's [Gunning Fog method](#sec-readability-test-options).
...
- Graph [watermarks](#sec-options-graphs) now support the tags `&[DATETIME]`, `&[DATE]`, and `&[TIME]`. These are expanded to their literal values at the time of rendering the graph.
...
## Readability Studio 3.0-3.2 (2009–2010) {-}
***
