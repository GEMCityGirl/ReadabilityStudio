...
- Meaningful names should always be used. (Only loop counters may have simple names like `i` ad `j`.)
...
- Instrinsic-type (and enumeration) member variables should be initialized.
- Macros should be avoided; prefer using functions, `constexpr`, `if constexpr`, etc.
...
- Exception messages should be translatable, unless in low-level code
  where `gettext` mechanisms aren't avaliable.
