...
################################################
files <- str_glue(
  "src/{list.files(path=srcFolder, pattern='(*[.]cpp|onelua_no_warnings[.]c)', recursive=TRUE)}")
# remove test and sample files from submodules
files <- files[!grepl("(wxSimpleJSON|reportbuilder.cpp|demo.cpp|main.cpp|Wisteria-Dataviz/tests|Catch2|OleanderStemmingLibrary/tests|tinyexpr-plusplus/tests|utfcpp/tests|utfcpp/samples|utfcpp/extern|cxxopts|xmltest.cpp|html5-printer.cpp)", files)]
