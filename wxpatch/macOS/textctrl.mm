...
@implementation NSTextViewWithHeadersAndFooters

- (NSAttributedString *) pageHeader
{
    NSMutableString* formattedHeader = [[[NSMutableString alloc] initWithString:[m_pageHeader string]] autorelease];
    
    NSString* pageNumberStr = [[[NSString alloc] initWithString:[NSString stringWithFormat:@"%ld", static_cast<unsigned long>([[NSPrintOperation currentOperation] currentPage])]] autorelease];
    [formattedHeader replaceOccurrencesOfString:@"@PN" withString:pageNumberStr
                                        options:0
                                          range:NSMakeRange(0, [formattedHeader length])];
    NSRange pageRange = NSMakeRange(NSNotFound,NSNotFound);
    pageRange = [[NSPrintOperation currentOperation] pageRange];
    NSString* pageCountStr = [[[NSString alloc] initWithString:[NSString stringWithFormat:@"%ld", static_cast<unsigned long>(pageRange.length)]] autorelease];
    [formattedHeader replaceOccurrencesOfString:@"@PC" withString:pageCountStr
                                        options:0
                                          range:NSMakeRange(0, [formattedHeader length])];
    NSMutableAttributedString* attributedFormattedHeader = [[[NSMutableAttributedString alloc] initWithString:formattedHeader] autorelease];
    [attributedFormattedHeader addAttribute:NSFontAttributeName
                                      value:[NSFont fontWithName:@"Courier New" size:12]
...
{
    NSMutableString* formattedHeader = [[[NSMutableString alloc] initWithString:[m_pageFooter string]] autorelease];
...
long wxNSTextViewControl::Find( const wxString &str, const bool caseSensitive, const bool searchForward, const bool wholeWord) const
{
    if (m_textView)
    {
        ...
        {
            area.location = to;
            area.length = [viewString length]-to;
        }
        else
        {
            area.location = 0;
        ...
        while (true)
        {
            found = [viewString rangeOfString:textContent
                                      options:searchFlags
                                        range:area];
            if (found.location == NSNotFound || !wholeWord)
            { break; }
            //if whole word searching, then we need determine that here
            if (found.length > 0)
            {
                //see if preceding character is alphanumeric or trailing character is alphanumeric
                if ((found.location > 0 && [[NSCharacterSet alphanumericCharacterSet] characterIsMember:[viewString characterAtIndex:found.location - 1]]) ||
                    (found.location + found.length < [viewString length] && [[NSCharacterSet alphanumericCharacterSet] characterIsMember:[viewString characterAtIndex:found.location + found.length]]) )
                {
                    if (searchForward)
                    {
                        area.location = found.location+found.length;
                        area.length = [viewString length]-area.location;
                    }
                    else
                    {
                        area.length = [viewString length]-found.location;
                    }
                    continue;
                }
                //otherwise, a whole word match
                else
                { break; }
            }
        ...
        return (found.location == NSNotFound) ? wxNOT_FOUND : found.location;
    }
    else
    {
        return wxNOT_FOUND;
