...
namespace LuaScripting
    {
    void VerifyLink(const wxChar* link, const size_t length, const bool isImage, const wxString& currentFile,
        ...
        //encoded links will hex encode non-alphanumeric values in them, so convert that to match the real name of the local file
        ...
        for (;;)
            {
            const int percentIndex = path.Find(wxT('%'));
            if (percentIndex == wxNOT_FOUND || static_cast<size_t>(percentIndex) > path.length()-3)
                { break; }
            size_t hexLength(static_cast<size_t>(-1));
            wxChar value = string_util::axtoi(path.Mid(percentIndex+1, 2).wc_str(), hexLength);
            path.replace(percentIndex, 3, wxString(1, value));
            }
        //strip off bookmark (if there is one)
        const int bookMarkIndex = path.Find(wxT('#'), true);
        ...
        //if link to a local CHM file then try to see if the decompiled file is in the local file system
        if (path.Find(_DT(L".chm::")) != wxNOT_FOUND ||
            path.Find(_DT(L".CHM::")) != wxNOT_FOUND)
            {
            path.Replace(_DT(L".chm::"), wxEmptyString);
            path.Replace(_DT(L".CHM::"), wxEmptyString);
            path.Prepend(wxFileName::GetCwd() + wxT('\\'));
        ...
        if (isImage)
            {
            const std::wstring widthStr = lily_of_the_valley::html_extract_text::read_attribute_as_string(link, _DT(L"width"), 5, true);
            const std::wstring heightStr = lily_of_the_valley::html_extract_text::read_attribute_as_string(link, _DT(L"height"), 6, true);
            if (widthStr.length() && heightStr.length())
                ...
                if (fn.FileExists())
                    ...
                    if (!img.IsOk())
                        { badImageSizes.insert(std::make_pair(currentFile, wxString::Format(_("%s (image is possibly corrupt))"), fn.GetFullPath()))); }
                    else if (!is_within<long>(img.GetHeight(), height-5, height+5) || !is_within<long>(img.GetWidth(), width-5, width+5) )
                        { badImageSizes.insert(std::make_pair(currentFile, wxString::Format(_("%s (%ld x %ld vs. %d x %d))"), fn.GetFullPath(), width, height, img.GetWidth(), img.GetHeight()))); }
                    }
                }
            }
        if (!fn.FileExists())
    ...
    int GetProgramPath(lua_State *L)
        {
        lua_pushstring(L, wxFileName(wxStandardPaths::Get().GetExecutablePath()).GetPath(wxPATH_GET_SEPARATOR|wxPATH_GET_VOLUME).utf8_str());
        return 1;
        }

    int GetLuaConstantsPath(lua_State *L)
        {
        lua_pushstring(L, wxGetApp().FindResourceFile(wxT("RSConstants.lua")).utf8_str());
        return 1;
        }
    ...
    int MsgBox(lua_State *L)
        ...
        if (lua_isboolean(L,1))
            ...
            wxMessageBox(lua_toboolean(L,1) ? _("true") : _("false"),
                (lua_gettop(L) > 1) ? wxString(luaL_checkstring(L, 2), wxConvUTF8) : wxGetApp().GetAppName(),
    ...
    int FindFiles(lua_State *L)
        ...
        wxDir dir;
        wxArrayString files;
        const size_t fileCount = dir.GetAllFiles(wxString(luaL_checkstring(L, 1), wxConvUTF8), &files, wxString(luaL_checkstring(L, 2), wxConvUTF8), wxDIR_FILES|wxDIR_DIRS);
    ...
    int SplashScreen(lua_State *L)
        ...
        const auto index = std::clamp<size_t>(lua_tonumber(L, 1)-1/*make it zero-indexed*/, 0, wxGetApp().GetSplashscreenPaths().GetCount()-1);
    ...
    int CheckHtmlLinks(lua_State *L)
        ...
        //get the valid files
            ...
            for (size_t i = 0; i < files.Count(); ++i)
                ...
                wxFFile file(files[i], wxT("r"));
                file.ReadAll(&fileContent, *wxConvCurrent);
                file.Close();

                int start = fileContent.find(wxT("[FILES]"));
                if (start == wxNOT_FOUND)
                    { continue; }
                start += 7;
                int end = fileContent.find(wxT("\n["), start);
                if (end == wxNOT_FOUND)
                    {
                    end = fileContent.find(wxT("\r["), start);
                    if (end == wxNOT_FOUND)
                        { continue; }
                    }
                fileContent = fileContent.Mid(start, end-start);

                wxStringTokenizer tkz(fileContent, wxT("\n\r"));
        ...
        for (size_t i = 0; i < files.Count(); ++i)
            ...
            while (foundBookMark.first)
                {
                foundBookMark = lily_of_the_valley::html_extract_text::find_bookmark(foundBookMark.first, htmlEnd);
            ...
            while ((currentLink = hparse()) != nullptr)
                ...
                if (hparse.get_current_hyperlink_length() > 2 && currentLink[0] == L'#')
                    {
                    wxString currentBookmark(currentLink+1, hparse.get_current_hyperlink_length()-1);
                    if (BookmarksInCurrentPage.find(currentBookmark) == BookmarksInCurrentPage.end())
                        {
                        badLinks.emplace(wxFileName(files[i]).GetFullName(), wxString(currentLink, hparse.get_current_hyperlink_length()));
                        }
                    }
                else
                    {
                    VerifyLink(currentLink, hparse.get_current_hyperlink_length(), hparse.is_current_link_an_image(), files[i],
                        badLinks, badImageSizes, IncludeExternalLinks);
                    }
                }
            }

        //analyze the TOCs and indices
        files.Clear();
        dir.GetAllFiles(path, &files, _DT(L"*.hh*"));
        for (size_t i = 0; i < files.Count(); ++i)
            {
            Wisteria::TextStream::ReadFile(files[i], fileContent);
            ...
            for (;;)
                {
                index = fileContent.find(_DT(L"param name=\"Local\" value=\""), index);
                if (index == wxNOT_FOUND)
                    { break; }
                index += 26;
                const auto endQuote = fileContent.find(wxT("\""), index);
                if (endQuote == wxNOT_FOUND)
                    { break; }
                wxString currentLink = fileContent.Mid(index, endQuote-index); 
                VerifyLink(currentLink, currentLink.length(), false, files[i], badLinks, badImageSizes, IncludeExternalLinks);
                }
            }
        //write out the warnings of bad links
        for (std::multimap<wxString,wxString>::const_iterator pos = badLinks.begin();
            pos != badLinks.end();
            ++pos)
            { wxLogError(L"%s: %s", pos->first, pos->second); }
        if (badLinks.size() == 0)
            { wxLogMessage(L"No bad links found in help folder."); }
        //write out the warnings of images whose size don't match their size specified in a topic
        for (std::multimap<wxString,wxString>::const_iterator pos = badImageSizes.begin();
            pos != badImageSizes.end();
            ++pos)
            { wxLogError(L"%s: %s", pos->first, pos->second); }
    ...
    int CrossReferenceWordLists(lua_State* L)
        ...
        for (std::vector<grammar::phrase_collection::phrase_word_pair>::const_iterator pos = phraseList.get_phrases().begin();
            pos != phraseList.get_phrases().end();
            ++pos)
            {
            tokenizer.SetString(pos->second.c_str(), wxT(","), wxTOKEN_STRTOK);
            while (tokenizer.HasMoreTokens())
                {
                wxString replacementStr = tokenizer.GetNextToken();
                replacementStr.Trim(true); replacementStr.Trim(false);
                if (replacementStr.find(wxT(' ')) == wxNOT_FOUND)
                    {
                    if (wordList.find(replacementStr))
                        {
                        outputStr += wxString::Format(wxT("%s\t%s\r\n"), pos->first.to_string().c_str(), replacementStr);
                        break;
                        }
                    }
                }
    ...
    int PhraseListToWordList(lua_State* L)
        ...
        for (auto pos = phraseList.get_phrases().cbegin();
            ...
            ++pos)
            {
            if (pos->first.get_word_count() == 1 && pos->first.get_type() == grammar::phrase_type::phrase_wordy && pos->second.length())
                { outputStr += wxString::Format(wxT("%s\t%s\r\n"), pos->first.to_string().c_str(), pos->second.c_str()); }
            }
    ...
    int ExpandWordList(lua_State* L)
        ...
        for (int i = 3; i <= lua_gettop(L); ++i)
            ...
            for (size_t j = 0; j < wordList.get_list_size(); ++j)
                ...
                if (itemToAppend.CmpNoCase(wxT("s")) == 0 &&
                    wordList.get_words().at(j).length() > 1)
                    {
                    const wchar_t secondToLastLetter = wordList.get_words().at(j).at(wordList.get_words().at(j).length()-2);
                    const wchar_t lastLetter = wordList.get_words().at(j).at(wordList.get_words().at(j).length()-1);
                    if ((secondToLastLetter == L's' || secondToLastLetter == L'S') &&
                        ...
                        {
                        newStrings.push_back(wordList.get_words().at(j).substr(0,wordList.get_words().at(j).length()-1)+_DT(L"ies"));
                        continue;
                        }
                    }
    ...
    int SetProjectLanguage(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetProjectLanguage(static_cast<readability::test_language>(static_cast<int>(lua_tonumber(L, 1))));
        wxGetApp().Yield();
        return 0;
        }
    ...
    int SetDocumentStorageMethod(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetDocumentStorageMethod(static_cast<TextStorage>(static_cast<int>(lua_tonumber(L, 1))));
        return 0;
        }
    ...
    int SetParagraphsParsingMethod(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetParagraphsParsingMethod(static_cast<ParagraphParse>(static_cast<int>(lua_tonumber(L, 1))));
        return 0;
        }
    int GetParagraphsParsingMethod(lua_State *L)
        {
        lua_pushnumber(L,static_cast<int>(wxGetApp().GetAppOptions().GetParagraphsParsingMethod()));
        return 1;
        }
    ...
    int GetMinDocWordCountForBatch(lua_State *L)
        {
        lua_pushnumber(L,static_cast<int>(wxGetApp().GetAppOptions().GetMinDocWordCountForBatch()));
        return 1;
        }
    int SetFilePathDisplayMode(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetFilePathTruncationMode(static_cast<ListCtrlEx::ColumnInfo::ColumnFilePathTruncationMode>(static_cast<int>(lua_tonumber(L, 1))));
        return 0;
        }
    ...
    int EnableWarning(lua_State* L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().EnableWarning(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        }
    ...
    int DisableWarning(lua_State* L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().DisableWarning(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        }
    ...
    int SetTextExclusion(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetInvalidSentenceMethod(static_cast<InvalidSentence>(static_cast<int>(lua_tonumber(L, 1))));
        return 0;
        }
    int SetIncludeIncompleteTolerance(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetIncludeIncompleteSentencesIfLongerThanValue(lua_tonumber(L, 1));
        return 0;
        }
    ...
    int ExcludeCopyrightNotices(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().IgnoreTrailingCopyrightNoticeParagraphs(int_to_bool(lua_toboolean(L, 1)));
        return 0;
        };
    ...
    int SetPhraseExclusionList(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetExcludedPhrasesPath(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        };
    int SetBlockExclusionTags(lua_State *L)
        ...
        if (exclusionTags.length() >= 2)
            { wxGetApp().GetAppOptions().GetExclusionBlockTags().push_back(std::make_pair(exclusionTags[0],exclusionTags[1])); }
        return 0;
        };
    ...
    int ApplyGraphBackgroundFade(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetGraphBackGroundLinearGradient(int_to_bool(lua_toboolean(L, 1)));
        return 0;
        }
    int SetGraphBackgroundImage(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetBackGroundImagePath(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        }
    ...
    int SetGraphCustomBrushImage(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetGraphStippleImagePath(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        }
    ...
    int SetBarChartBarEffect(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetGraphBarEffect(static_cast<BoxEffect>(static_cast<int>(lua_tonumber(L, 1))));
        return 0;
        }
    int SetBarChartOrientation(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetBarChartOrientation(static_cast<Wisteria::Orientation>(static_cast<int>(lua_tonumber(L, 1))));
        return 0;
        }
    ...
    int SetSpellCheckerOptions(lua_State *L)
        {
        if (lua_gettop(L) >= 1)
            { wxGetApp().GetAppOptions().SpellCheckIgnoreProperNouns(int_to_bool(lua_toboolean(L, 1))); }
        if (lua_gettop(L) >= 2)
        ...
        if (lua_gettop(L) >= 4)
            { wxGetApp().GetAppOptions().SpellCheckIgnoreFileAddresses(int_to_bool(lua_toboolean(L, 4))); }
        if (lua_gettop(L) >= 5)
            { wxGetApp().GetAppOptions().SpellCheckIgnoreProgrammerCode(int_to_bool(lua_toboolean(L, 5))); }
        wxGetApp().Yield();
        return 0;
        }
    ...
    int SetWindowSize(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 2, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetMainFrame()->SetSize(lua_tonumber(L, 1), lua_tonumber(L, 2));
    ...
    int SetLeftPrintHeader(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetLeftPrinterHeader(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        }
    int SetCenterPrintHeader(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetCenterPrinterHeader(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        }
    int SetRightPrintHeader(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetRightPrinterHeader(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        }
    ...
    int SetLeftPrintFooter(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetLeftPrinterFooter(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        }
    int SetCenterPrintFooter(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetCenterPrinterFooter(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        }
    int SetRightPrintFooter(lua_State *L)
        ...
        if (!VerifyParameterCount(L, 1, __WXFUNCTION__))
            { return 0; }
        wxGetApp().GetAppOptions().SetRightPrinterFooter(wxString(luaL_checkstring(L, 1), wxConvUTF8));
        return 0;
        }
    }
