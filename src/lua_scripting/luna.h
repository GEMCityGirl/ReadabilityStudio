...
public:
    // REGISTER CONSTRUCTOR AS A GLOBAL TABLE via __call function
    ...
    static void RegisterTable(lua_State * L, const char *namespac = NULL) {
    ...
    */
    static int property_getter(lua_State * L)
    {
        ...
        if (lua_isnumber(L, -1)) { // Check if we got a valid index
            ...
            int _index = (int)lua_tointeger(L, -1);
