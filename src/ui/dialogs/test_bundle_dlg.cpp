...
TestBundleDlg::TestBundleDlg(wxWindow* parent, TestBundle& testBundle) :
    ...
    wxIcon ico;
    ico.CopyFromBitmap(
        wxGetApp().GetResourceManager().GetSVG(L"ribbon/bundles.svg").
...
void TestBundleDlg::CreateControls()
    ...
    // custom tests
    const auto includedCustomTests = std::count_if(m_custom_tests.cbegin(), m_custom_tests.cend(),
    ...
    if (BaseProject::m_custom_word_tests.size() &&
        ...
        for (auto& cTest : m_custom_tests)
            ...
            cTest.include(m_testBundle.GetTestGoals().find(
                TestGoal{ cTest.get_name().c_str() }) != m_testBundle.GetTestGoals().end());
            // add the test to the dialog
            wxCheckBox* testCheckBox = new wxCheckBox(page, wxID_ANY, cTest.get_name().c_str(),
                wxDefaultPosition, wxDefaultSize, 0, wxGenericValidator(&cTest.get_include_flag()) );
            if (m_testBundle.IsLocked())
                { testCheckBox->Disable(); }
    ...
    // Vocabulary suites
    // Dolch
    m_includeDolchSightWords = (m_testBundle.GetTestGoals().find(
        TestGoal{ ReadabilityMessages::DOLCH().wc_str() }) != m_testBundle.GetTestGoals().end());
    if ((m_testBundle.GetLanguage() == readability::test_language::unknown_language ||
        ...
        // Test goals
            ...
            if (!m_testBundle.IsLocked())
                ...
                Bind(wxEVT_BUTTON,
                    [this](wxCommandEvent&)
                        {
                        m_testGoalsListCtrl->DeleteSelectedItems();
                        },
                    ...
                    if (found)
                        ...
                        if (!std::isnan(bundleTest.GetMinGoal()))
                            ...
                            m_testGoalsListCtrl->SetItemText(insertedItem, 1,
                                wxNumberFormatter::ToString(bundleTest.GetMinGoal(), 1,
                                    wxNumberFormatter::Style::Style_NoTrailingZeroes));
                            }
                        if (!std::isnan(bundleTest.GetMaxGoal()))
                            {
                            m_testGoalsListCtrl->SetItemText(insertedItem, 2,
                                wxNumberFormatter::ToString(bundleTest.GetMaxGoal(), 1,
                                    wxNumberFormatter::Style::Style_NoTrailingZeroes));
                            }
                        }
                    // or a custom one
        ...
        // Stat goals
            ...
            if (!m_testBundle.IsLocked())
                ...
                Bind(wxEVT_BUTTON,
                    [this](wxCommandEvent&)
                        {
                        m_statGoalsListCtrl->DeleteSelectedItems();
                        },
                    ...
                    if (statPos != BaseProject::GetStatGoalLabels().cend())
                        ...
                        if (!std::isnan(bundleStat.GetMinGoal()))
                            ...
                            m_statGoalsListCtrl->SetItemText(insertedItem, 1,
                                wxNumberFormatter::ToString(bundleStat.GetMinGoal(), 1,
                                    wxNumberFormatter::Style::Style_NoTrailingZeroes));
                            }
                        if (!std::isnan(bundleStat.GetMaxGoal()))
                            {
                            m_statGoalsListCtrl->SetItemText(insertedItem, 2,
                                wxNumberFormatter::ToString(bundleStat.GetMaxGoal(), 1,
                                    wxNumberFormatter::Style::Style_NoTrailingZeroes));
                            }
                        }
                    }
...
void TestBundleDlg::OnOK([[maybe_unused]] wxCommandEvent& event)
    ...
    if (m_includeDolchSightWords)
        { m_testBundle.GetTestGoals().insert(
            TestGoal{ReadabilityMessages::DOLCH().wc_str()}); }
