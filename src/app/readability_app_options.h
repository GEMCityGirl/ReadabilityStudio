...
///Class for managing what is included in the statistics reports. By default, all statistics are included (except additional notes).
...
public:
    ...
    [[nodiscard]] bool HasDolchStatisticsEnabled() const
...
public:
    ...
    [[nodiscard]] bool IsLongSentencesEnabled() const
    ...
    [[nodiscard]] bool IsLengthsBoxPlotEnabled() const
    ...
    [[nodiscard]] bool IsLengthsHeatmapEnabled() const
    ...
    [[nodiscard]] bool IsLengthsHistogramEnabled() const
...
public:
    ...
    [[nodiscard]] TextHighlight GetTextHighlightMethod() const noexcept
    ...
    [[nodiscard]] wxColour GetTextHighlightColor() const
    ...
    [[nodiscard]] wxColour GetExcludedTextHighlightColor() const
    ...
    [[nodiscard]] wxColour GetDuplicateWordHighlightColor() const
    ...
    [[nodiscard]] wxColour GetWordyPhraseHighlightColor() const
   ...
   [[nodiscard]]  bool IsHighlightingDolchVerbs() const noexcept
    ...
    [[nodiscard]] bool IsIgnoringProperNouns() noexcept
    ...
    void SetTextSource(const TextSource value) noexcept
        { m_textSource = value; }
    //Test Recommendation
    [[nodiscard]] TestRecommendation GetTestRecommendation() noexcept
        { return m_testRecommendation; }
    void SetTestRecommendation(const TestRecommendation value) noexcept
        { m_testRecommendation = value; }
    //Tests by Industry
    [[nodiscard]] readability::industry_classification GetTestByIndustry() const noexcept
    ...
    [[nodiscard]] readability::document_classification GetTestByDocumentType() const noexcept
    ...
    // tests by bundle
    [[nodiscard]] const wxString& GetSelectedTestBundle() const noexcept
    ...
    //Window information
    [[nodiscard]] bool IsAppWindowMaximized() const noexcept
        { return m_appWindowMaximized; }
    void SetAppWindowMaximized(const bool maximized) noexcept
        { m_appWindowMaximized = maximized; }
    [[nodiscard]] int GetAppWindowWidth() const noexcept
        { return m_appWindowWidth; };
    [[nodiscard]] int GetAppWindowHeight() const noexcept
    ...
    [[nodiscard]] wxColour GetStartPageDetailBackgroundColor() const
    ...
    [[nodiscard]] bool IsLicenseAccepted() const noexcept
    ...
    [[nodiscard]] wxString GetImagePath() const
        { return m_imagePath; }
    void SetImagePath(const wxString& path)
        { m_imagePath = path; }
    [[nodiscard]] wxString GetProjectPath() const
        { return m_projectPath; }
    void SetProjectPath(const wxString& path)
        { m_projectPath = path; }
    [[nodiscard]] wxString GetWordListPath() const
    ...
    [[nodiscard]] wxColour GetPlotBackGroundColor() const
    ...
    [[nodiscard]] wxString GetWatermarkLogo() const
    ...
    [[nodiscard]] wxString GetGraphStippleImagePath() const
    ...
    [[nodiscard]] size_t GetBatchRandomSamplingSize() const noexcept
    ...
    void SetReviewer(const wxString& reviewer)
        { m_reviewer = reviewer; }
    [[nodiscard]] wxString GetReviewer() const
        { return m_reviewer; }
    void SetStatus(const wxString& status)
        { m_status = status; }
    [[nodiscard]] wxString GetStatus() const
        { return m_status; }
    //printer settings
    //page setup
    void SetPaperId(const int Id) noexcept
        { m_paperId = Id; }
    [[nodiscard]] int GetPaperId() const noexcept
        { return m_paperId; }

    void SetPaperOrientation(const wxPrintOrientation orient) noexcept
        { m_paperOrientation = orient; }
    [[nodiscard]] wxPrintOrientation GetPaperOrientation() const noexcept
    ...
    [[nodiscard]] std::vector<wxColour>& GetCustomColours() noexcept
...
private:
    ...
    //Tests by Industry
    readability::industry_classification m_testsByIndustry{ readability::industry_classification::adult_publishing_industry };
    //Tests By Document Type
    readability::document_classification m_testsByDocumentType{ readability::document_classification::adult_literature_document };
    ...
    //Histogram options
    Wisteria::Graphs::Histogram::BinningMethod m_histogramBinningMethod{ Wisteria::Graphs::Histogram::BinningMethod::BinByIntegerRange };
    Wisteria::BinLabelDisplay m_histrogramBinLabelDisplayMethod{ Wisteria::BinLabelDisplay::BinValue };
...
public:
    ...
    const wxString XML_GRAPH_PLOT_BACKGROUND_OPACITY;
    ...
    //meta statistics
    const wxString XML_OFFICE_DOCUMENT_META ;
    const wxString XML_OFFICE_DOCUMENT_META_HEADER;
    const wxString XML_OFFICE_META;
    const wxString XML_STATISTICS;
    const wxString XML_TEXT_SIZE;
    const wxString XML_UNIQUE_WORDS;
    const wxString XML_UNIQUE_MONOSYLLABLIC_WORDS;
    const wxString XML_UNIQUE_SIXCHAR_PLUS_WORDS;
    const wxString XML_UNIQUE_THREE_SYLLABLE_PLUS_WORDS;
    const wxString XML_UNIQUE_DC_HARD_WORDS;
    const wxString XML_UNIQUE_SPACHE_HARD_WORDS;
    const wxString XML_UNIQUE_HARRIS_JACOBSON_HARD_WORDS;
    const wxString XML_UNIQUE_MINI_WORDS;
    const wxString XML_UNIQUE_FOG_HARD_WORDS;
    const wxString XML_UNIQUE_SMOG_WORDS_WORDS;
    const wxString XML_TOTAL_WORDS;
    const wxString XML_TOTAL_SENTENCES;
    const wxString XML_TOTAL_SENTENCE_UNITS;
    const wxString XML_TOTAL_PARAGRAPH;
    const wxString XML_TOTAL_SYLLABLE;
    const wxString XML_TOTAL_CHARACTERS;
    const wxString XML_TOTAL_CHARACTERS_PLUS_PUNCTUATION;
    const wxString XML_TOTAL_MONOSYLLABIC_WORDS;
    const wxString XML_TOTAL_THREE_PLUSSYLLABLE_WORDS;
    const wxString XML_TOTAL_SPACHE_HARD_WORDS;
    const wxString XML_TOTAL_HARRIS_JACOBSON_HARD_WORDS;
    const wxString XML_TOTAL_DALECHALL_HARD_WORDS;
    const wxString XML_TOTAL_FOG_HARDWORDS;
    const wxString XML_TOTAL_SMOG_HARD_WORDS;
    const wxString XML_TOTAL_SOL_HARD_WORDS;
    const wxString XML_TOTAL_LIX_RIX_HARD_WORDS;
    const wxString XML_TOTAL_SYLLABLES_IGNORING_NUMERALS;
    const wxString XML_TOTAL_SYLLABLES_NUMERALS_ONE_SYLLABLE;
    const wxString XML_TOTAL_SYLLABLES_NUMERALS_FULLY_SYLLABIZED;
    const wxString XML_TOTAL_SYLLABLES_IGNORING_NUMERALS_AND_PROPER_NOUNS;
    const wxString XML_TOTAL_LONG_WORDS;
    const wxString XML_TOTAL_LONG_WORDS_IGNORING_NUMERALS;
    const wxString XML_TOTAL_MINIWORDS;
    const wxString XML_TOTAL_NUMERALS;
    const wxString XML_TOTAL_PROPER_NOUNS;
    const wxString XML_TOTAL_OVERLY_LONG_SENTENCES;
    const wxString XML_TOTAL_INTERROGATIVE_SENTENCES;
    const wxString XML_TOTAL_EXCLAMATORY_SENTENCES;
    const wxString XML_TOTAL_WORDS_FROM_COMPLETE_SENTENCES_AND_HEADERS;
    const wxString XML_TOTAL_SENTENCES_FROM_COMPLETE_SENTENCES_AND_HEADERS;
    const wxString XML_TOTAL_NUMERALS_FROM_COMPLETE_SENTENCES_AND_HEADERS;
    const wxString XML_TOTAL_CHARACTERS_FROM_COMPLETE_SENTENCES_AND_HEADERS;
    const wxString XML_LONGEST_SENTENCES;
    const wxString XML_LONGEST_SENTENCE_INDEX;
    const wxString XML_DIFFICULT_SENTENCE_LENGTH;
    const wxString XML_DUPLICATE_WORD_COUNT;
    const wxString XML_MISSPELLING_COUNT;
    const wxString XML_WORDY_PHRASE_COUNT;
    const wxString XML_CLICHE_COUNT;
    const wxString XML_REDUNDANT_PHRASE_COUNT;
    const wxString XML_WORDING_ERROR_COUNT;
    const wxString XML_ARTICLE_MISMATCH_COUNT;
    const wxString XML_PASSIVE_VOICE_COUNT;
    const wxString XML_OVERUSED_WORDS_BY_SENTENCE_COUNT;
    const wxString XML_SENTENCE_CONJUNCTION_START_COUNT;
    const wxString XML_SENTENCE_LOWERCASE_START_COUNT;
    const wxString XML_DOLCH_UNIQUE_CONJUNCTIONS_COUNT;
    const wxString XML_DOLCH_UNIQUE_PREPOSITIONS_COUNT;
    const wxString XML_DOLCH_UNIQUE_PRONOUNS_COUNT;
    const wxString XML_DOLCH_UNIQUE_ADVERBS_COUNT;
    const wxString XML_DOLCH_UNIQUE_ADJECTIVES_COUNT;
    const wxString XML_DOLCH_UNIQUE_VERBS_COUNT;
    const wxString XML_DOLCH_UNIQUE_NOUNS_COUNT;
    const wxString XML_DOLCH_TOTAL_CONJUNCTIONS_COUNT;
    const wxString XML_DOLCH_TOTAL_PREPOSITIONS_COUNT;
    const wxString XML_DOLCH_TOTAL_PRONOUNS_COUNT;
    const wxString XML_DOLCH_TOTAL_ADVERBS_COUNT;
    const wxString XML_DOLCH_TOTAL_ADJECTIVES_COUNT;
    const wxString XML_DOLCH_TOTAL_VERBS_COUNT;
    const wxString XML_DOLCH_TOTAL_NOUNS_COUNT;
    const wxString XML_DOLCH_UNUSED_CONJUNCTIONS_COUNT;
    const wxString XML_DOLCH_UNUSED_PREPOSITIONS_COUNT;
    const wxString XML_DOLCH_UNUSED_PRONOUNS_COUNT;
    const wxString XML_DOLCH_UNUSED_ADVERBS_COUNT;
    const wxString XML_DOLCH_UNUSED_ADJECTIVES_COUNT;
    const wxString XML_DOLCH_UNUSED_VERBS_COUNT;
    const wxString XML_DOLCH_UNUSED_NOUNS_COUNT;
    const wxString XML_UNIQUE_UNFAMILIAR_WORD_COUNT;
    const wxString XML_UNFAMILIAR_WORD_COUNT;
    //export options
