...
#include "../../Wisteria-Dataviz/src/util/stringutil.h"
...
public:
    ...
    /// Negative cloze scores make little sense, but are published in the New DC workbook, so allow them
    [[nodiscard]]
    ...
    /// @param format A wxNumberFormatInfo object, specifying the precision to format the number into.
    ///     This only applies if using short format.
    /// @returns The grade level, formatted as a string.
    [[nodiscard]]
    wxString GetFormattedValue(const wxString& value, const Wisteria::NumberFormatInfo& format) const final;
    ...
    /// @returns A long-format string value of a grade, regardless of whether long formatting is enabled
    [[nodiscard]]
