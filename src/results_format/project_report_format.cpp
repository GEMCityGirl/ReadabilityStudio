    ...
    if (statsInfo.IsDolchCoverageEnabled())
        ...
        const double dolchNounPercentage =
            ...
            if (listData)
                ...
                listData->SetItemText(
                    listDataItemCount++, 2,
                    wxNumberFormatter::ToString(dolchConjuctionPercentage, 1,
                                                wxNumberFormatter::Style::Style_NoTrailingZeroes) +
                        L"% " + _(L"of all Dolch conjunctions"));
                }
            }
            ...
            const wxString valueStr =
                (dolchPrepositionsPercentage >= 75) ?
                    wxString::Format(L"<span style=\"color:%s\">",
                                     attentionColor.GetAsString(wxC2S_HTML_SYNTAX)) +
                        wxNumberFormatter::ToString(MAX_DOLCH_PREPOSITION_WORDS -
                                                        project->GetUnusedDolchPrepositions(),
                                                    0, true) +
                        L"</span>" :
                    wxNumberFormatter::ToString(
...
wxString ProjectReportFormat::FormatHtmlReportStart([[maybe_unused]] const wxColour bgColor,
    ...
    {
    wxString cssTemplatePath = wxGetApp().FindResourceDirectory(_DT(L"ReportThemes")) +
...
wxString ProjectReportFormat::FormatStatisticsInfo(
            ...
            currentValue = (project->GetLongestSentence() >= 30) ?
                               ...
                               wxNumberFormatter::ToString(
                                   ...
                                   wxNumberFormatter::Style::Style_NoTrailingZeroes |
                                       wxNumberFormatter::Style::Style_WithThousandsSep);
            HTMLText +=
                formatRow(wxString::Format(_(L"Longest sentence (sentence #%s):"),
                                           // add 1 to display it as 1 based
                                           wxNumberFormatter::ToString(
                                               project->GetLongestSentenceIndex() + 1, 0, true)),
                          currentValue);
    ...
    if (statsInfo.IsExtendedWordsEnabled())
        ...
        HTMLText += tableStart + formatHeader(_(L"Monosyllabic Words")) +
                    formatRow(_(L"Number of monosyllabic words:"),
                              wxNumberFormatter::ToString(
                                  project->GetTotalMonoSyllabicWords(), 0,
                                  wxNumberFormatter::Style::Style_NoTrailingZeroes |
                                      wxNumberFormatter::Style::Style_WithThousandsSep),
                              L"(" +
                                  wxNumberFormatter::ToString(
    ...
    if (statsInfo.IsNotesEnabled() &&
        (project->GetInvalidSentenceMethod() == InvalidSentence::ExcludeFromAnalysis ||
         ...
         averageSyllableCount >= 4 || project->GetAppendedDocumentFilePath().length()))
        {
