    ...
    for (std::vector<grammar::paragraph_info>::const_iterator para_iter = theDocument->get_paragraphs().begin();
        ...
        for (size_t j = para_iter->get_first_sentence_index();
            ...
            const bool currentSentenceShouldBeHighlightedAsInvalid =
                ...
                (highlightIncompleteSentences &&
                    !considerOnlyListItemsAsCompleteSentences && !currentSentence.is_valid());
            if (currentSentenceShouldBeHighlightedAsInvalid)
                {
                std::wcsncpy(text+documentTextLength, IGNORE_HIGHLIGHT_BEGIN, IGNORE_HIGHLIGHT_BEGIN.length() );
            ...
            for (size_t i = currentSentence.get_first_word_index();
                ...
                if (currentSentence.is_valid() && highlightInvalidWords &&
                    ...
                    std::wcsncpy(text+documentTextLength,
                        IGNORE_HIGHLIGHT_BEGIN.wc_str(), IGNORE_HIGHLIGHT_BEGIN.length() );
                    documentTextLength += IGNORE_HIGHLIGHT_BEGIN.length();
                ...
                // or highlight if this word meets our criteria for highlighting
                    ...
                    {
                    std::wcsncpy(text+documentTextLength, shouldHighlight.GetHightlightBegin().wc_str(),
                        shouldHighlight.GetHightlightBegin().length() );
                    documentTextLength += shouldHighlight.GetHightlightBegin().length();
                    std::wcsncpy(text+documentTextLength, currentWord.c_str(), currentWord.length() );
                    documentTextLength += currentWord.length();
                    std::wcsncpy(text+documentTextLength, shouldHighlight.GetHightlightEnd().wc_str(),
                ...
                while (punctPos != punctEnd &&
                    ...
                    if (i == currentSentence.get_last_word_index() &&
                        ...
                        if (characters::is_character::is_quote(punctPos->get_punctuation_mark()))
                            {
                            std::wcsncpy(text+documentTextLength,
                                endingPunctuation.c_str(), endingPunctuation.length() );
            ...
            if (currentSentenceShouldBeHighlightedAsInvalid)
                {
                std::wcsncpy(text+documentTextLength, IGNORE_HIGHLIGHT_END, IGNORE_HIGHLIGHT_END.length() );
                documentTextLength += IGNORE_HIGHLIGHT_END.length();
                }
...
static size_t FormatWordCollectionHighlightedGrammarIssues(const documentT* theDocument,
    ...
    // whether we are in phrase highlight mode and the "countdown" value (0 means to close the highlighting)
    ...
    for (auto para_iter = theDocument->get_paragraphs().cbegin();
        ...
        for (size_t j = para_iter->get_first_sentence_index();
            ...
            if (j >= theDocument->get_sentences().size())
                { continue; } // this should not happen, this is just a sanity trap
            const grammar::sentence_info currentSentence = theDocument->get_sentences().at(j);
            const size_t currentSentenceLength =
                highlightInvalidWords ?
                currentSentence.get_valid_word_count() : currentSentence.get_word_count();
            const bool currentSentenceShouldBeHighlightedAsInvalid =
                    ...
                    if (wordyIndicesIter != theDocument->get_known_phrase_indices().end() &&
                        ...
                        if (wordyPhrases[wordyIndicesIter->second].first.get_type() ==
                            ...
                            std::wcsncpy(text+documentTextLength, PHRASE_HIGHLIGHT_BEGIN.wc_str(),
                                PHRASE_HIGHLIGHT_BEGIN.length());
                            documentTextLength += PHRASE_HIGHLIGHT_BEGIN.length();
                            }
                        currentPhraseMode.first = true;
                        currentPhraseMode.second =
                            static_cast<int>(wordyPhrases[wordyIndicesIter->second].first.get_word_count());
                        }
                    }
                // passive voice
                if (!currentPhraseMode.first)
        ...
        for (size_t j = para_iter->get_first_sentence_index();
            ...
            for (size_t i = currentSentence.get_first_word_index();
                ...
                while (punctPos != punctEnd &&
                    ...
                    std::wcsncpy(text+documentTextLength, punct.wc_str(), punct.length() );
                    documentTextLength += punct.length();
                    ++punctPos;
                    }
