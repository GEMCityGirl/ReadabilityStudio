...
namespace grammar
    ...
    protected:
        ...
        [[nodiscard]] static std::pair<bool,size_t> is_special_math_word(const wchar_t* start, const size_t length) noexcept;
        ...
        [[nodiscard]] static size_t syllabify_numeral(const wchar_t* numeral_string,
        ...
        [[nodiscard]] static size_t get_symbol_syllable_count(const wchar_t* start,
                                                const wchar_t* end,
        ...
        /** @brief For possessive words (ape's), this will adjust the length to ignore the possessive part of the word.
            @note This adjustment is done by shortening the internal length variable, it does not change the text buffer.
        ...
        /** @brief If there are any dashes in this word, then break it up into smaller words.
            ...
            @param start The start of the word.*/
        template<typename Tsyllabizer>
        [[nodiscard]] bool syllabize_if_contains_dashes(const wchar_t* start)
            {
        ...
        [[nodiscard]] bool is_consonant_y(const wchar_t* word, size_t position) const;
    ...
    class syllabize_english_number
        {
    public:
        [[nodiscard]] constexpr size_t operator()(const wchar_t number) const noexcept
            {
            return ((number >= common_lang_constants::NUMBER_1 && number <= common_lang_constants::NUMBER_6) || (number >= common_lang_constants::NUMBER_1_FULL_WIDTH && number <= common_lang_constants::NUMBER_6_FULL_WIDTH)) ?
                    1 : ((number == common_lang_constants::NUMBER_8 || number == common_lang_constants::NUMBER_9) || (number == common_lang_constants::NUMBER_8_FULL_WIDTH || number == common_lang_constants::NUMBER_9_FULL_WIDTH)) ?
                    1 : ((number == common_lang_constants::NUMBER_0 || number == common_lang_constants::NUMBER_7) || (number == common_lang_constants::NUMBER_0_FULL_WIDTH || number == common_lang_constants::NUMBER_7_FULL_WIDTH)) ?
                    2 : 0;
            }
        };
    ...
    private:
        /// @brief Analyzes the overall string for special situations that they standard syllabizer would have missed.
        ...
        [[nodiscard]] bool is_silent_u(const wchar_t* word, size_t position) const;
        //--------------------------------------------------
        [[nodiscard]] bool does_prefix_silence_e(const wchar_t* word, size_t prefix_length) const;
        //--------------------------------------------------
        [[nodiscard]] static bool does_prefix_disconnect_e(const wchar_t* word, size_t prefix_length);
        //--------------------------------------------------
        [[nodiscard]] static bool is_none_affecting_suffix(const wchar_t* word, size_t suffix_length);
        /// @returns @c true if suffix makes the preceding 'e' NOT silent
        [[nodiscard]] bool does_suffix_negate_silent_e(const wchar_t* suffix, const size_t suffix_length,
        ...
        [[nodiscard]] bool is_single_non_e_vowel_ignored(const wchar_t* word, const size_t position) const;
        /** @brief Determines if an 'e' at given position is silent.
            @param word The current word being analyzed (must be the start of the word).
            @param position The position of the vowel.
            @returns @c true if 'e' at @position is silent.*/
        [[nodiscard]] bool is_silent_e(const wchar_t* word, const size_t position) const;
        //--------------------------------------------------
        [[nodiscard]] bool is_vowels_separate_syllables(const wchar_t* word,
        ...
        [[nodiscard]] static bool can_consonants_end_sound(const wchar_t* consonants, size_t block_length);
        //--------------------------------------------------
        [[nodiscard]] static bool can_consonants_be_modified_by_following_e(const wchar_t* consonants, size_t block_length);
        //--------------------------------------------------
        [[nodiscard]] static bool can_consonants_begin_sound(const wchar_t* consonants, size_t block_length);
        ...
        [[nodiscard]] static std::pair<size_t,size_t> get_prefix_length(const wchar_t* start, const size_t length);
