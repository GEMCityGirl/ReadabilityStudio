...
bool is_abbreviation::operator()(const std::wstring_view text) const
    ...
    if (characters::is_character::is_consonant(text[0]))
        {
        size_t i = 1;
        // go through everything (except the last character) and stop if we hit something other than
        // a consonant
        for (i = 1; i < text.length() - 1; ++i)
            ...
            if (text.length() >= 5 &&
                traits::case_insensitive_ex::eq(text[i], common_lang_constants::PERIOD) && i > 1)
                {
                return true;
                }
