...
wxBEGIN_EVENT_TABLE(ProjectView, BaseProjectView)
    ...
    EVT_LIST_ITEM_ACTIVATED(ExplanationListCtrl::EXPLANATION_LIST_CTRL_GRID, ProjectView::OnTestListDblClick)
...
void ProjectView::OnExportFilteredDocument([[maybe_unused]] wxCommandEvent& event)
    ...
    FilteredTextPreviewDlg dlg(GetDocFrame(), doc->GetInvalidSentenceMethod(), doc->IsIgnoringTrailingCopyrightNoticeParagraphs(), doc->IsIgnoringTrailingCitations(),
...
void ProjectView::OnTestListDblClick([[maybe_unused]] wxListEvent& event)
    ...
    if (testIter != BaseProject::m_custom_word_tests.end())
        ...
        { wxGetApp().GetMainFrame()->DisplayHelp(wxString::Format(wxT("%s.html"), testPos.first->get_id().c_str())); }
    else if (selectedTest == _("Dolch Sight Words"))
        { wxGetApp().GetMainFrame()->DisplayHelp(wxT("reviewing-dolch.html.html")); }
    }
...
void ProjectView::UpdateSideBarIcons()
    ...
    if (GetReadabilityResultsView().GetWindowCount() > 0)
        ...
        for (size_t i = 0; i < GetReadabilityResultsView().GetWindowCount(); ++i)
            ...
            GetSideBar()->InsertSubItemById(SIDEBAR_READABILITY_SCORES_SECTION_ID, GetReadabilityResultsView().GetWindow(i)->GetName(),
                GetReadabilityResultsView().GetWindow(i)->GetId(),
                GetReadabilityResultsView().GetWindow(i)->GetId() == READABILITY_SCORES_SUMMARY_REPORT_PAGE_ID ? 17 :
                GetReadabilityResultsView().GetWindow(i)->GetId() == READABILITY_SCORES_PAGE_ID ? 23 :
                GetReadabilityResultsView().GetWindow(i)->GetId() == READABILITY_GOALS_PAGE_ID ? 28 :
                (isGraph &&
                     typeid(*dynamic_cast<Wisteria::Canvas*>(GetReadabilityResultsView().GetWindow(i))->GetFixedObject(0,0)) == typeid(FleschChart)) ? 18 :
                (isGraph &&
                     typeid(*dynamic_cast<Wisteria::Canvas*>(GetReadabilityResultsView().GetWindow(i))->GetFixedObject(0,0)) == typeid(FraseGraph)) ? 19 :
                (isGraph &&
                     typeid(*dynamic_cast<Wisteria::Canvas*>(GetReadabilityResultsView().GetWindow(i))->GetFixedObject(0,0)) == typeid(FryGraph)) ? 20 :
                (isGraph &&
                     typeid(*dynamic_cast<Wisteria::Canvas*>(GetReadabilityResultsView().GetWindow(i))->GetFixedObject(0,0)) == typeid(RaygorGraph)) ? 21 :
                (isGraph &&
                     typeid(*dynamic_cast<Wisteria::Canvas*>(GetReadabilityResultsView().GetWindow(i))->GetFixedObject(0,0)) == typeid(CrawfordGraph)) ? 22 :
                (isGraph &&
                     typeid(*dynamic_cast<Wisteria::Canvas*>(GetReadabilityResultsView().GetWindow(i))->GetFixedObject(0,0)) == typeid(SchwartzGraph)) ? 25 :
                (isGraph &&
                     typeid(*dynamic_cast<Wisteria::Canvas*>(GetReadabilityResultsView().GetWindow(i))->GetFixedObject(0,0)) == typeid(LixGauge)) ? 26 :
                (isGraph &&
                     typeid(*dynamic_cast<Wisteria::Canvas*>(GetReadabilityResultsView().GetWindow(i))->GetFixedObject(0,0)) == typeid(LixGaugeGerman)) ? 26 :
                (isGraph &&
                     typeid(*dynamic_cast<Wisteria::Canvas*>(GetReadabilityResultsView().GetWindow(i))->GetFixedObject(0,0)) == typeid(DanielsonBryan2Plot)) ? 27 :
                    9);
            }
        }
        ...
        for (size_t i = 0; i < GetWordsBreakdownView().GetWindowCount(); ++i)
            ...
            GetSideBar()->InsertSubItemById(SIDEBAR_WORDS_BREAKDOWN_SECTION_ID, GetWordsBreakdownView().GetWindow(i)->GetName(),
                ...
                GetWordsBreakdownView().GetWindow(i)->IsKindOf(CLASSINFO(FormattedTextCtrl)) ? 0 : 
                    ...
                    (isGraph &&
                        typeid(*dynamic_cast<Wisteria::Canvas*>(GetWordsBreakdownView().GetWindow(i))->GetFixedObject(0,0)) == typeid(Wisteria::Graphs::BarChart)) ? 16 :
                    (isGraph &&
                        typeid(*dynamic_cast<Wisteria::Canvas*>(GetWordsBreakdownView().GetWindow(i))->GetFixedObject(0,0)) == typeid(Wisteria::Graphs::WordCloud)) ? 29 :
                    9);
            }
        }
    ...
    if (GetSentencesBreakdownView().GetWindowCount() > 0)
        ...
        for (size_t i = 0; i < GetSentencesBreakdownView().GetWindowCount(); ++i)
            ...
            GetSideBar()->InsertSubItemById(SIDEBAR_SENTENCES_BREAKDOWN_SECTION_ID, GetSentencesBreakdownView().GetWindow(i)->GetName(),
                ...
                (isGraph &&
                    typeid(*dynamic_cast<Wisteria::Canvas*>(GetSentencesBreakdownView().GetWindow(i))->GetFixedObject(0,0)) == typeid(Wisteria::Graphs::BoxPlot)) ? 7 :
                (isGraph &&
                    typeid(*dynamic_cast<Wisteria::Canvas*>(GetSentencesBreakdownView().GetWindow(i))->GetFixedObject(0,0)) == typeid(Wisteria::Graphs::Histogram)) ? 6 :
                (isGraph &&
                    typeid(*dynamic_cast<Wisteria::Canvas*>(GetSentencesBreakdownView().GetWindow(i))->GetFixedObject(0,0)) == typeid(Wisteria::Graphs::HeatMap)) ? 24 :
                9);
            }
        }
...
void ProjectView::OnMenuCommand(wxCommandEvent& event)
    ...
    if (event.GetId() == XRCID("ID_SAVE_ITEM") )
        ...
        std::dynamic_pointer_cast<Wisteria::Graphs::BarChart>(
            barChart->GetFixedObject(0,0))->SortBars(BarChart::BarSortComparison::SortByBarLength, Wisteria::SortDirection::SortAscending);
        barChart->Refresh();
        barChart->Update();
        return;
        }
        ...
        Wisteria::Canvas* barChart = dynamic_cast<Wisteria::Canvas*>(GetActiveProjectWindow());
        std::dynamic_pointer_cast<Wisteria::Graphs::BarChart>(
            barChart->GetFixedObject(0,0))->SortBars(BarChart::BarSortComparison::SortByBarLength, Wisteria::SortDirection::SortDescending);
        barChart->Refresh();
        barChart->Update();
        return;
        }
    ...
    if (GetActiveProjectWindow() && GetActiveProjectWindow()->IsKindOf(CLASSINFO(ListCtrlEx)) )
        ...
        list->SetLabel(wxString::Format(wxT("%s [%s]"), list->GetName(), wxFileName::StripExtension(doc->GetTitle())));
        ...
        html->SetLabel(wxString::Format(wxT("%s [%s]"), html->GetName(), wxFileName::StripExtension(doc->GetTitle())));
        ...
        elist->SetLabel(wxString::Format(wxT("%s [%s]"), elist->GetName(), wxFileName::StripExtension(doc->GetTitle())));
        ...
        graph->SetLabel(wxString::Format(wxT("%s [%s]"), graph->GetName(), wxFileName::StripExtension(doc->GetTitle())));
...
bool ProjectView::OnCreate(wxDocument* doc, long flags)
    ...
    readabilityScoresView->GetExplanationView()->SetPage(wxString(wxT("<html><body>")) + _("No readability test results currently available.") + wxString(wxT("</body></html>")));
    readabilityScoresView->GetResultsListCtrl()->AssignContextMenu(wxXmlResource::Get()->LoadMenu(wxT("IDM_READABILITY_SCORE_LIST")) );
    readabilityScoresView->GetExplanationView()->AssignContextMenu(wxXmlResource::Get()->LoadMenu(wxT("IDM_HTML_MENU_NO_SAVE")) );
    ...
    m_menuBar = wxXmlResource::Get()->LoadMenuBar(wxT("ID_DOCMENUBAR"));
#endif
...
void ProjectView::UpdateStatistics()
    ...
    if (rowCount > 1)
        ...
        for (int i = 0; i < rowCount; ++i)
            {
            double value = 0;
            if (ReadabilityMessages::GetScoreValue(GetReadabilityScoresList()->GetResultsListCtrl()->GetItemTextEx(i, 1), value))
                { grades.push_back(value); }
            if (ReadabilityMessages::GetScoreValue(GetReadabilityScoresList()->GetResultsListCtrl()->GetItemTextEx(i, 2), value))
                { ages.push_back(value); }
            if (ReadabilityMessages::GetScoreValue(GetReadabilityScoresList()->GetResultsListCtrl()->GetItemTextEx(i, 4), value))
        ...
        wxString explanationString = wxT("<table class='minipage' style='width:100%;'>") +
        ...
        explanationString += wxT("</span></td></tr></thead>\n\t<tr><td>");
        ...
        if (doc->GetStatisticsReportInfo().IsExtendedInformationEnabled())
            ...
            explanationString = wxT("<table class='minipage' style='width:100%;'>") +
                wxString::Format(wxT("\n\t<thead><tr><td style='background:%s;'><span style='color:%s;'>"),
                    ProjectReportFormat::GetReportHeaderColor().GetAsString(wxC2S_HTML_SYNTAX),
                    ProjectReportFormat::GetReportHeaderFontColor().GetAsString(wxC2S_HTML_SYNTAX));
...
void ProjectView::OnLongFormat([[maybe_unused]] wxRibbonButtonBarEvent& event)
    ...
    GetReadabilityScoresList()->GetResultsListCtrl()->SetColumnWidth(1, GetReadabilityScoresList()->GetResultsListCtrl()->EstimateColumnWidth(1));
    doc->SetModifiedFlag();
    }
...
void ProjectView::OnTestDelete([[maybe_unused]] wxRibbonButtonBarEvent& event)
    ...
    if (GetActiveProjectWindow() && GetActiveProjectWindow()->GetId() != READABILITY_SCORES_PAGE_ID)
        {
        const auto [parentId, childId] = GetSideBar()->FindSubItem(SIDEBAR_READABILITY_SCORES_SECTION_ID, READABILITY_SCORES_PAGE_ID);
    ...
    if (selectedIndex != wxNOT_FOUND)
        ...
        UpdateStatistics();
        GetReadabilityScoresList()->GetExplanationView()->SetPage(wxEmptyString);
        ...
        if (GetReadabilityScoresList()->GetResultsListCtrl()->GetItemCount() == 0)
            {
            GetReadabilityScoresList()->GetExplanationView()->SetPage(wxString(wxT("<html><body>")) + _("No readability test results currently available.") + wxString(wxT("</body></html>")));
            }
...
void ProjectView::OnItemSelected(wxCommandEvent& event)
    ...
    const auto readRibbonButtonSVG = [this](const auto& path)
        ...
        return wxGetApp().GetResourceManager().
            GetSVG(path).GetBitmap(GetActiveProjectWindow()->FromDIP(wxSize(32, 32))).
        ...
        if (GetActiveProjectWindow())
            ...
            if (GetRibbon())
                ...
                if (editButtonBarWindow && editButtonBarWindow->IsKindOf(CLASSINFO(wxRibbonButtonBar)))
                    ...
                    switch (event.GetInt())
                        {
                    case READABILITY_SCORES_PAGE_ID:
                        exportMenuIcon = L"scores-window";
                        break;
                    case READABILITY_SCORES_SUMMARY_REPORT_PAGE_ID:
                        exportMenuIcon = L"report";
                        break;
                    case READABILITY_GOALS_PAGE_ID:
                        exportMenuIcon = "checklist";
                        break;
                        }
                    if (exportMenuIcon.length())
                        {
                        exportMenuItem->SetBitmap(
                            wxGetApp().GetResourceManager().GetSVG(
                                wxString::Format(L"ribbon/%s.svg", exportMenuIcon)));
                        }
            ...
            if (GetRibbon())
                ...
                if (editButtonBarWindow && editButtonBarWindow->IsKindOf(CLASSINFO(wxRibbonButtonBar)))
                    ...
                    switch (event.GetInt())
                        {
                    case FLESCH_CHART_PAGE_ID:
                        exportImg = L"flesch-test";
                        break;
                    case DB2_PAGE_ID:
                        exportImg = L"danielson-bryan-2";
                        break;
                    case FRY_PAGE_ID:
                        exportImg = L"fry-test";
                        break;
                    case RAYGOR_PAGE_ID:
                        exportImg = L"raygor-test";
                        break;
                    case CRAWFORD_GRAPH_PAGE_ID:
                        exportImg = L"crawford";
                        break;
                    case FRASE_PAGE_ID:
                        exportImg = L"frase";
                        break;
                    case SCHWARTZ_PAGE_ID:
                        exportImg = L"schwartz";
                        break;
                    case LIX_GAUGE_PAGE_ID:
                        exportImg = L"lix-test";
                        break;
                    case LIX_GAUGE_GERMAN_PAGE_ID:
                        exportImg = L"lix-german-technical";
                        break;
                    case GPM_FRY_PAGE_ID:
                        exportImg = L"gilliam-pena-mountain-fry-graph";
                        break;
                        }
                    if (exportImg.length())
                        {
                        exportItem->SetBitmap(
                            wxGetApp().GetResourceManager().GetSVG(wxString::Format(L"tests/%s.svg", exportImg)));
                        }
                    ...
                    const auto graphType = dynamic_cast<Wisteria::Canvas*>(GetActiveProjectWindow())->GetFixedObject(0,0);
...
bool ProjectView::ExportAll(const wxString& folder, wxString listExt, wxString textExt, wxString graphExt,
    ...
    if (textExt.empty())
        { textExt = wxT(".htm"); }
    else if (textExt[0] != wxT('.'))
        { textExt.insert(0, wxT(".")); }
                ...
                if (activeWindow)
                    ...
                    if (activeWindow->IsKindOf(CLASSINFO(ExplanationListCtrl)) )
                        ...
                        list->Save(
                            ...
                            list->GetLabel() + wxT(".htm"), ExplanationListExportOptions::ExportGrid);
                        }
                        ...
                        reportWindow->SetLabel(wxString::Format(wxT("%s [%s]"), reportWindow->GetName(), wxFileName::StripExtension(doc->GetTitle())));
                        reportWindow->Save(folder + wxFileName::GetPathSeparator() + GetReadabilityScoresLabel() + wxFileName::GetPathSeparator() + 
                            reportWindow->GetLabel() + wxT(".htm"));
                        }
...
bool ProjectView::ExportAllToHtml(const wxFileName& filePath, wxString graphExt,
    ...
    if (includeTestScores && GetReadabilityResultsView().GetWindowCount())
        ...
        outputText += wxT("\n\n<div class='report-section'><a name='scores'></a>") +
            htmlEncode({ GetReadabilityScoresLabel().wc_str() }, true) + wxT("</div>\n");
        for (size_t i = 0; i < GetReadabilityResultsView().GetWindowCount(); ++i)
            {
            wxWindow* activeWindow = GetReadabilityResultsView().GetWindow(i);
            if (activeWindow)
    ...
    if (includeStatistics && GetSummaryView().GetWindowCount())
        { TOC += wxT("<a href='#stats'>") + GetSummaryStatisticsLabel() + wxT("</a><br />\n"); }
    if (includeWordsBreakdown && GetWordsBreakdownView().GetWindowCount())
        { TOC += wxT("<a href='#wordsbreakdown'>") + GetWordsBreakdownLabel() + wxT("</a><br />\n"); }
    if (includeSentencesBreakdown && GetSentencesBreakdownView().GetWindowCount())
        { TOC += wxT("<a href='#sentencesbreakdown'>") + GetSentencesBreakdownLabel() + wxT("</a><br />\n"); }
