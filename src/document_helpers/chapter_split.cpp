...
bool ChapterSplit::SplitHtmlByBookmarks(wxString& sourceFile, const bool convertToXhtml)
    {
    wxString outputFolder = wxFileName(sourceFile).GetPath(wxPATH_GET_VOLUME|wxPATH_GET_SEPARATOR)+wxFileName(sourceFile).GetName();
    if (!wxFileName::DirExists(outputFolder))
        {
        if (outputFolder.IsEmpty() || !wxFileName::Mkdir(outputFolder, wxS_DIR_DEFAULT, wxPATH_MKDIR_FULL) )
            { return false; }
        }
    ...
    wxString headSection = wxT("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>\n<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.1//EN\" \"http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd\">\n<html xmlns=\"http://www.w3.org/1999/xhtml\" xml:lang=\"en\">\n<head>\n<meta content=\"text/html; charset=UTF-8\"/>\n<title></title>\n<link href=\"stylesheet.css\" type=\"text/css\" rel=\"stylesheet\" />\n<link rel=\"stylesheet\" type=\"application/vnd.adobe-page-template+xml\" href=\"page-template.xpgt\"/>\n</head>");
    const wchar_t* headStart = html_extract_text::find_element(htmlText, htmlTextEnd, _DT(L"head"), 4);
    if (!convertToXhtml && headStart)
        {
        const wchar_t* headEnd = html_extract_text::find_closing_element(headStart, htmlTextEnd, _DT(L"head"), 4);
        if (headEnd)
            {
        ...
        headSection.Prepend(wxT("<html>\n"));
        }
    //read the Body declaration
    wxString bodySection = wxT("<body>");
    const wchar_t* bodyStart = html_extract_text::find_element(htmlText, htmlTextEnd, _DT(L"body"), 4);
    ...
    std::pair<const wchar_t*,std::wstring> bookmark = html_extract_text::find_bookmark(htmlText, htmlTextEnd);
    while (bookmark.first)
        {
        std::pair<const wchar_t*,std::wstring> nextBookMark = html_extract_text::find_bookmark(bookmark.first+1, htmlTextEnd);
        ...
        if (nextBookMark.first)
            ...
            if (outputText.length())
                {
                outputText.insert(0,
                    wxString::Format(wxT("%s\n%s\n<div id=\"%s\">"), headSection, bodySection, bookmark.second.c_str()));
                outputText.append(L"\n</div>\n</body>\n</html>");
                string_util::replace_all(outputText, std::wstring(L"<mbp:pagebreak />"), std::wstring(L""));
                lily_of_the_valley::html_format::set_encoding(outputText, wxT("UTF-8"));
                wxFile fout(wxString::Format(wxT("%s\\%s.xhtml"), outputFolder, bookmark.second.c_str()), wxFile::write);
            ...
            if (outputText.length())
                ...
                outputText.insert(0,
                    wxString::Format(wxT("%s\n%s\n<div id=\"%s\">"), headSection, bodySection, bookmark.second.c_str()));
                string_util::replace_all(outputText, std::wstring(L"<mbp:pagebreak />"), std::wstring(L""));
                const size_t endBodyTag = outputText.find(wxT("</body>"));
...
void ChapterSplit::WriteChapterFile(const wchar_t* textStart, const wchar_t* textEnd)
    ...
    while (*endOfFirstLine && (endOfFirstLine< textEnd) && !isEol(endOfFirstLine[0]))
        { ++endOfFirstLine; }
    const wxString chapterName = StripIllegalFileCharacters(wxString(firstLine, (endOfFirstLine-firstLine)));
    wxFile ff(m_outputFolder + wxFileName::GetPathSeparator() + chapterName + L".txt", wxFile::write);
    if (ff.IsOpened())
        {
    #ifdef __WXMSW__
