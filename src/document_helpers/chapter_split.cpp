...
bool ChapterSplit::SplitHtmlByBookmarks(wxString& sourceFile, const bool convertToXhtml)
    ...
    auto bookmark =
        html_extract_text::find_bookmark(htmlText, htmlTextEnd);
    while (bookmark.first)
        {
        auto nextBookMark =
            html_extract_text::find_bookmark(bookmark.first+1, htmlTextEnd);
        const wchar_t* endOfFirstBookmark =
            html_extract_text::find_close_tag(bookmark.first);
        // shouldn't happen
        ...
        if (nextBookMark.first)
            ...
            if (outputText.length())
                ...
                outputText.insert(0,
                    wxString::Format(L"%s\n%s\n<div id=\"%s\">",
                        headSection, bodySection, bookmark.second.c_str()));
                outputText.append(L"\n</div>\n</body>\n</html>");
                string_util::replace_all(outputText,
                    std::wstring(L"<mbp:pagebreak />"), std::wstring{});
...
void ChapterSplit::WriteChapterFile(const wchar_t* textStart, const wchar_t* textEnd)
    ...
    wxFile ff(m_outputFolder + wxFileName::GetPathSeparator() +
              chapterName + L".txt", wxFile::write);
    if (ff.IsOpened())
        {
    #ifdef __WXMSW__
